...
and cannot receive invitations. Everything is managed programmatically by an
admin using the :ref:`Management SDK <enterprise-management-sdk>`.
...
Service accounts are managed entirely through the Management SDK. The following
examples cover the full lifecycle:
...
Service accounts authenticate via API keys, which are generated by an admin
