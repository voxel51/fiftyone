...
You can use the
:meth:`evaluate_vqa() <fiftyone.core.collections.SampleCollection.evaluate_vqa>`
method to evaluate the predicted answers stored in a :ref:`VQA or VQAs <vqa>`
field of your dataset with respect to ground truth answers stored in another
field of the same type.

Within |VQAs| fields, predictions are matched to ground truth questions by
their
:attr:`question_id <fiftyone.core.labels.VQA.question_id>` attributes;
labels without a ``question_id`` are matched positionally.

Invoking
:meth:`evaluate_vqa() <fiftyone.core.collections.SampleCollection.evaluate_vqa>`
returns a |VQAResults| instance that provides an
:attr:`accuracy <fiftyone.utils.eval.vqa.VQAResults.accuracy>` property, a
:meth:`breakdown() <fiftyone.utils.eval.vqa.VQAResults.breakdown>` method
that groups scores by
:attr:`question_type <fiftyone.core.labels.VQA.question_type>` or
:attr:`answer_type <fiftyone.core.labels.VQA.answer_type>`, and the standard
classification report and confusion matrix methods over the observed answers.

In addition, when you specify an ``eval_key`` parameter, some helpful fields
are recorded that you can leverage via the :ref:`FiftyOne App <fiftyone-app>`
to interactively explore the strengths and weaknesses of your model on
individual questions:

-   the per-sample mean question score in an ``eval_key`` sample field
-   the per-question score in an ``eval_key`` attribute of each ground truth
    and predicted label (boolean for ``"exact"``, float for ``"vqa"``)
-   the ID of the matched counterpart label in an ``eval_key + "_id"``
    attribute of each matched label

Exact match evaluation (default)
--------------------------------

By default,
:meth:`evaluate_vqa() <fiftyone.core.collections.SampleCollection.evaluate_vqa>`
scores a prediction as correct if its
:attr:`answer <fiftyone.core.labels.VQA.answer>` exactly matches the ground
truth ``answer`` (or, if unset, any entry of the ground truth
:attr:`answers <fiftyone.core.labels.VQA.answers>`), after both are passed
through the official VQAv2 answer normalizer (lowercasing, punctuation and
article removal, number-word mapping, contraction expansion). You can
disable normalization by passing ``normalize=False``.

You can explicitly request that exact match evaluation be used by setting
the ``method`` parameter to ``"exact"``.

The example below demonstrates exact match evaluation on a small toy
dataset:
...
.. code-block:: python
    ...
    dataset.add_samples(
        [
            fo.Sample(
                ...
                predictions=fo.VQAs(
                    vqas=[
                        fo.VQA(answer="yes", question_id="q1", confidence=0.9),
                        fo.VQA(answer="two", question_id="q2", confidence=0.8),
                    ]
                ),
            ),
            fo.Sample(
                ...
                predictions=fo.VQAs(
                    vqas=[
                        fo.VQA(answer="black", question_id="q3", confidence=0.4)
                    ]
                ),
            ),
        ]
    ...
    # Evaluate the predictions in the `predictions` field with respect to the
    # answers in the `ground_truth` field
    ...
    # The mean per-question score
    # Note that "two" matches "2" via the answer normalizer
    print(results.accuracy)
    # 0.6666666666666666

    # Mean scores by question type
    print(results.breakdown(by="question_type"))
    # {'how many': 1.0, 'is there': 1.0, 'what color': 0.0}

    # Print a classification report over the observed answers
    results.print_report()
...
VQA accuracy protocol
---------------------

Open-ended VQA benchmarks in the VQAv2 family provide multiple reference
answers per question and score predictions with a soft accuracy that awards
partial credit based on annotator agreement. You can apply this protocol by
setting the ``method`` parameter to ``"vqa"``.

Each prediction is scored against the ground truth
:attr:`answers <fiftyone.core.labels.VQA.answers>` list (falling back to
``[answer]``) via the official protocol: hold out each reference in turn,
count matches among the remaining references, apply ``min(1, matches / 3)``,
and average over all holdouts. Following the official implementation,
answers are normalized only when the references disagree:
...
.. code-block:: python
    ...
    dataset.add_sample(
        fo.Sample(
            ...
            predictions=fo.VQAs(
                vqas=[fo.VQA(answer="parasol", question_id="q1")]
            ),
        )
    )
    ...
    # "parasol" was given by 2 of 10 annotators, so the prediction earns
    # partial credit under the min(1, matches / 3) holdout protocol
    print(results.accuracy)
    # 0.6
...
.. note::
    ...
    yet support ``"vqa"`` runs. You can triage errors in the App via the
    eval fields that this method populates — for example, by filtering the
    predictions field to labels whose ``eval_key`` attribute is ``False``.
