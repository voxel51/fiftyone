...
When the scrolling through the grid, a certain number samples are cached by the
App to improve the navigation experience. The number of samples is thresholded
by a size estimate in megabytes. The default grid cache size is 1/8 of your
device's memory and only includes samples that are not currently visible on
your screen.
...
maps. To disable autosizing, you can toggle the setting in the settings cog or
simply zoom back out with the slider setting.
