    ...
    def _make_dataset_cards(self, datasets: List[HFDataset]) -> str:
        ...
        for idx, dataset in enumerate(datasets):
            ...
            with open(
                ...
                note = """```{note}
This is a **Hugging Face dataset**. For large datasets, ensure `huggingface_hub>=1.1.3` to avoid rate limits. You can also set your `HF_TOKEN` for higher limits. Learn more in the <a href="https://docs.voxel51.com/integrations/huggingface.html#loading-datasets-from-the-hub" target="_blank">Hugging Face integration docs</a>.
```
