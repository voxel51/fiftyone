...
test(`ego default group slice transitions`, async ({ grid, modal }) => {
  ...
  await modal.sidebarPom.assert.verifySidebarEntryText("group.name", "ego");
  await modal.clickOnLooker();
  await modal.sidebarPom.assert.verifySidebarEntryText("group.name", "left");
  await modal.navigateSlice("group", "right", true);
  await modal.sidebarPom.assert.verifySidebarEntryText("group.name", "right");
