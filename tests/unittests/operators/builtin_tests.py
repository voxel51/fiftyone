...
def test_resolve_input_with_no_indexes(ctx):
...
def test_resolve_input_with_non_unique_indexes(ctx):
    ctx.dataset.get_index_information.return_value = {
...
def test_execute_with_missing_index_name(ctx):
    ctx.params = {}
    op = ConvertIndexToUnique()
...
def test_execute_with_nonexistent_index(ctx):
...
def test_execute_dry_run_success(ctx):
...
def test_execute_dry_run_violation(ctx):
    index_name = "some_index"
    ctx.params = {"index_name": index_name, "dry_run": True}
    ctx.dataset.get_index_information.return_value = {
        index_name: {"key": [("some_field", 1)]}
