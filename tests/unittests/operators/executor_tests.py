...
class TestActiveSurface(unittest.TestCase):
    def test_active_surface_from_string(self):
        ...
        self.assertEqual(
            ctx.active_surface, types.OperatorSurface.DATASET_SAMPLE_MODAL
        )

    def test_active_surface_from_enum(self):
        ctx = ExecutionContext(
            request_params={
                "active_surface": types.OperatorSurface.DATASET_SAMPLES_GRID
            }
        )
        self.assertEqual(
            ctx.active_surface, types.OperatorSurface.DATASET_SAMPLES_GRID
        )
...
async def test_execute_operator_receives_active_surface(list_operators):
    ...
    assert isinstance(result, ExecutionResult)
    assert (
        result.result["surface"] == types.OperatorSurface.DATASET_SAMPLE_MODAL
    )
