...
// DEBUG duplicate-request audit: redundant traffic fails loudly (full-screen
// overlay + thrown error with the caller's stack) instead of silently costing
// the backend. Scope: samples/frames/graphql/events/aggregations + media.
// Disable with `window.__foDupeGuardOff = true`.
// Guard state lives on window: survives HMR module re-evaluation (no reset or
// double-registration mid-session), resets exactly on a hard refresh
type DupeGuardState = {
  seenRequests: Map<string, number>;
  coveredWindows: Map<string, Array<[number, number]>>;
  eventsSubscriptions: Map<string, Set<string>>;
  activeEventStreams: Map<string, number>;
  seenMedia: Map<string, number>;
  mediaObserverInstalled?: boolean;
};
const guardState = (): DupeGuardState => {
  const host = window as { __foDupeGuardState?: DupeGuardState };
  host.__foDupeGuardState ??= {
    seenRequests: new Map(),
    coveredWindows: new Map(),
    eventsSubscriptions: new Map(),
    activeEventStreams: new Map(),
    seenMedia: new Map(),
  };
  return host.__foDupeGuardState;
};
const assertNotDuplicate = (method: string, path: string, body: unknown) => {
  if (
    typeof window === "undefined" ||
    (window as { __foDupeGuardOff?: boolean }).__foDupeGuardOff ||
    // audit scope: sample reads, frames, graphql, events, aggregations
    !/samples|frames|graphql|events|aggregat/.test(path)
  ) {
    return;
  }
  ...
  // events: repeat polls for ONE subscription are the protocol; a SECOND
  // subscription for the same event set is a duplicate
  const subscribed = body as { subscription?: string } | null;
  if (/events|event\b/.test(path) && subscribed?.subscription) {
    const { subscription, ...rest } = subscribed;
    let strippedKey: string;
    try {
      strippedKey = `${method} ${path} ${JSON.stringify(rest)}`;
    } catch {
      return;
    }
    const { eventsSubscriptions } = guardState();
    const subscriptions = eventsSubscriptions.get(strippedKey) ?? new Set();
    if (subscriptions.size > 0 && !subscriptions.has(subscription)) {
      failLoudly(
        `[dupe-guard] duplicate events subscription (${subscriptions.size + 1} ` +
          `subscriptions for one event set): ${method} ${path} :: ${strippedKey.slice(0, 300)}`,
      );
    }
    subscriptions.add(subscription);
    eventsSubscriptions.set(strippedKey, subscriptions);
    return;
  }

  const windowed = body as {
    after?: number;
    count?: number;
  ...
  if (windowed && typeof windowed.count === "number") {
    const start = (typeof windowed.after === "number" ? windowed.after : 0) + 1;
    const end = start + windowed.count - 1;
    let scope: string;
    try {
      scope = `${method} ${path} ${JSON.stringify([
        windowed.dynamicGroup,
        windowed.view,
        windowed.filters,
        windowed.filter,
      ])}`;
    ...
    const { coveredWindows } = guardState();
    const windows = coveredWindows.get(scope) ?? [];
    const clash = windows.find(([s, e]) => start <= e && end >= s);
    if (clash) {
      failLoudly(
        `[dupe-guard] window [${start}, ${end}] overlaps already-requested ` +
          `[${clash[0]}, ${clash[1]}]: ${method} ${path} :: ${serialized.slice(0, 300)}`,
      );
    }
    windows.push([start, end]);
    coveredWindows.set(scope, windows);
    return;
  }

  const key = `${method} ${path} ${serialized}`;
  const { seenRequests } = guardState();
  const count = (seenRequests.get(key) ?? 0) + 1;
  seenRequests.set(key, count);
  if (count > 1) {
    failLoudly(
      `[dupe-guard] duplicate request #${count}: ${method} ${path} :: ${serialized.slice(0, 300)}`,
    );
  }
};

// full-screen takeover: some callers swallow rejections, and a duplicate must
// break EVERYTHING visibly, stack included
const failLoudly = (message: string): never => {
  const error = new Error(message);
  console.error(error);
  const overlay = document.createElement("div");
  overlay.style.cssText =
    "position:fixed;inset:0;z-index:2147483647;background:#111;color:#ff6b6b;" +
    "font:13px/1.6 monospace;padding:32px;overflow:auto;white-space:pre-wrap";
...
// media loads go through <img>/media elements, never fetch(); observe resource
// timings so a repeated download of the same media URL fails the audit too
if (
  typeof window !== "undefined" &&
  "PerformanceObserver" in window &&
  !guardState().mediaObserverInstalled
) {
  guardState().mediaObserverInstalled = true;
  try {
    new PerformanceObserver((list) => {
      const { seenMedia } = guardState();
      for (const entry of list.getEntries()) {
        // the media endpoint only — not source modules named media.*
        if (!/\/media\?/.test(entry.name)) {
          continue;
        }
        // browser-cache replays (transferSize 0) never reach the backend; only
        // real network transfers count toward duplicates
        if ((entry as PerformanceResourceTiming).transferSize === 0) {
          continue;
        }
        const count = (seenMedia.get(entry.name) ?? 0) + 1;
        seenMedia.set(entry.name, count);
        if (
          count > 1 &&
          !(window as { __foDupeGuardOff?: boolean }).__foDupeGuardOff
        ) {
          failLoudly(
            `[dupe-guard] duplicate media request #${count}: ${entry.name.slice(0, 300)}`,
          );
        }
      }
    }).observe({ type: "resource", buffered: true });
  } catch {
    // resource timing unavailable; media stays unaudited
  }
}

// concurrent SSE event streams carrying the same payload are duplicates; a
// reconnect after abort is not (the abort listener releases the slot)
export const assertSingleEventStream = (
  path: string,
  body: unknown,
  signal: AbortSignal,
) => {
  ...
  let key: string;
  try {
    const { subscription: _subscription, ...rest } = (body ?? {}) as {
      subscription?: string;
    };
    key = `${path} ${JSON.stringify(rest)}`;
  } catch {
    return;
  }
  const { activeEventStreams } = guardState();
  const active = (activeEventStreams.get(key) ?? 0) + 1;
  activeEventStreams.set(key, active);
  signal.addEventListener(
    "abort",
    () =>
      activeEventStreams.set(
        key,
        Math.max(0, (activeEventStreams.get(key) ?? 1) - 1),
      ),
    { once: true },
  );
  if (active > 1) {
    failLoudly(
      `[dupe-guard] duplicate event stream #${active}: ${key.slice(0, 300)}`,
    );
  }
};
...
export const setFetchFunction = (
  ...
  const fetchFunctionExtended: FetchFunctionExtended = async (
    ...
    assertNotDuplicate(method, path, body);
...
export const getEventSource = (
  ...
  if (polling) {
    ...
    assertSingleEventStream(path, body, signal);
