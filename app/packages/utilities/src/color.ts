...
import { overlayPool } from "@voxel51/voodo/tokens";
 ...
 *
 * This is Voodo's `viz-overlay` group, not `viz-chart`. These colors are drawn
 * *on media* — detection boxes, masks, keypoints — and the image behind them
 * does not get lighter when the UI does, so they are identical in both themes.
 * That is also why the server can hold the same list with no notion of theme:
 * `fiftyone/_voodoo_palette.py` is generated from this same group, and
 * `tools/sync_voodoo_palette.py --check` fails if the two drift.
 */
export const default_app_color = overlayPool;
