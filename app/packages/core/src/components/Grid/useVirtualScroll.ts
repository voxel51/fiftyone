...
// keeps `moving` true across frame gaps so a flick never momentarily reads as settled.
const SETTLE_DELAY_MS = 140;
const VELOCITY_SMOOTHING = 0.7; // EMA factor smoothing per-frame wheel-notch spikes
...
export default function useVirtualScroll({
  ...
  const ensureLoop = useCallback(() => {
    ...
    const tick = () => {
      ...
      // hysteresis bridges frame gaps so both flags never flicker mid-flick.
      ...
      // stop only once settled so the loop never leaves `moving` stuck true.
  ...
  // preventDefault so the page doesn't also scroll; OS inertia gives flick for free.
  useEffect(() => {
  ...
  // clamp vTop into a shrunken extent on resize / zoom change.
  useEffect(() => {
