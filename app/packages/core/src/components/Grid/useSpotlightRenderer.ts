...
// The Spotlight engine's item renderer (auto-AR path): show/hide/detach callbacks
// over the shared looker cache. The fixed-AR engine uses ./useRenderer instead.
export default function useSpotlightRenderer({
  cache,
  id,
  records,
  store,
  ...
  const showItem = useCallback<Show<number, fos.Sample>>(
    ({ id, element, dimensions, spotlight, zooming }) => {
      ...
      item.addEventListener("selectthumbnail", ({ detail }: CustomEvent) =>
