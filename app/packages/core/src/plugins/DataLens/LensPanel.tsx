...
export const LensPanel = () => {
    ...
    const listConfigsOperator = useOperatorExecutor('@voxel51/operators/lens_list_lens_configs');
    const searchOperator = useOperatorExecutor('@voxel51/operators/lens_datasource_connector');

    const openDatasetOperator = useOperatorExecutor('@voxel51/operators/open_dataset');
    ...
    const handleLensConfigsUpdate = (configs: LensConfig[]) => {
        ...
        const isActiveConfigValid = activeConfig?.id
    ...
    const doImport = async () => {
        ...
        const batchSize = Math.max(1, Math.floor(maxBatchBytes / averageSampleSize));
    ...
    if (isInitializing) {
        ...
        return (
            <LensConfigManager configs={lensConfigs}
        ...
        return (
            <EmptyState
                onManageConfigsClick={() => setShowConfigManager(true)}
            />
        );
    } else {
        // Render main content.
        ...
        const lensConfigContent = (
            <Box sx={{m: 2}}>
                <Typography sx={{mb: 2}}>
                    Select datasource configuration
                </Typography>
        ...
        const queryOperatorContent = (
            <Box sx={{m: 2}}>
                ...
                <Box sx={{mb: 2, border: '1px solid #333', p: 2}}>
                    <OperatorConfigurator operator={activeConfig.operator_uri}
                                          formState={formState}
        ...
        const searchControls = (
            <Box sx={{m: 2, mt: 3}}>
                <Box sx={{
                    ...
                    <FormControl>
                        <TextField variant='outlined' label='Number of samples'
                                   value={maxSamples}
                                   onChange={e => setMaxSamples(isNaN(Number.parseInt(e.target.value)) ? 10 : Number.parseInt(e.target.value))}/>
                    </FormControl>
                </Box>
            </Box>
        );
        ...
        const previewContent = searchResponse ? (
            <>
                <Box sx={{m: 2}}>
                    <Box sx={{mt: 1}}>
                        <Box sx={{
                            ...
                            <Typography>
                                {searchResponse.result_count.toLocaleString(undefined, {maximumFractionDigits: 0})}
                                {' '}samples received in{' '}
                ...
                {searchResponse.result_count > 0 &&
                    <Box sx={{m: 2}}>
                        <Lens samples={searchResponse.query_result}
            ...
            <Box sx={{m: 2}}>
                ...
                <Box sx={{maxWidth: '750px', m: 'auto', p: 2}}>
                    ...
                    <Typography sx={{textAlign: 'center', mb: 6}} variant='h6'>
                        Search your connected datasources directly from
                        FiftyOne
                    </Typography>
