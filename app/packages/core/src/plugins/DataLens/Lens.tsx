...
export const Lens = (
    ...
    const spotlight = useMemo(() => {
        ...
        return new Spotlight<number, fos.Sample>({
            ...
            render: (id: ID, element: HTMLDivElement, dimensions: [number, number], soft: boolean, disable: boolean) => {
                ...
                if (!(createLooker.current && sample)) {
                    throw new Error(`createLooker=${!!createLooker.current}, sample=${JSON.stringify(sample)}`);
                }
    ...
    return (
        // Container element
        <Box sx={{border: '1px solid #777', width: '100%', height: '800px', p: 2}}>
            {/*// Spotlight*/}
