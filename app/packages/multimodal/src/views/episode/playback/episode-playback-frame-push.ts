...
export function pushTickToStore(
  ...
  lastFrame: Map<string, EpisodeStreamPlaybackFrame<unknown>>,
  store: PlaybackStore,
  ...
  for (const stream of activeStreams) {
    ...
    if (msg !== undefined) {
      setEpisodeStreamDiagnostics(store, stream, msg?.output.diagnostics ?? []);
    }
    let toWrite: EpisodeStreamPlaybackFrame<unknown> | null;
    if (msg === undefined) {
