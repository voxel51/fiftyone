...
describe("useTemporalTags", () => {
  describe("tracks", () => {
    it("returns an empty array when there are no temporal tags", () => {
      const { result } = renderHook(() => useTemporalTags(ctx));
    ...
    it("assigns a deterministic color based on the label", () => {
      ...
      const { result: r1 } = renderHook(() => useTemporalTags(ctx));
      const { result: r2 } = renderHook(() => useTemporalTags(ctx));
