...
import { getStreamDiagnostics } from "./stream-status-state";
...
function frameAt(store: PlaybackStore) {
  return getStreamValue<StreamPlaybackFrame<unknown> | null>(store, STREAM);
}
...
describe("pushTickToStore", () => {
  ...
  it("publishes capability diagnostics from attributes-only messages", () => {
    ...
    expect(frameAt(store)).toBeNull();
    expect(getStreamDiagnostics(store, STREAM)).toEqual([
