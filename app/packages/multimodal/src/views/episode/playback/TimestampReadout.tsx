...
export function getInferredTimeZone(): string {
  return INFERRED_TIME_ZONE;
}

export function searchTimeZones(search: string): string[] {
...
export function formatTimeZoneOption(timeZone: string, ns: bigint): string {
...
function getTimeZoneTree(ns: bigint): TimeZoneTreeRoot {
  ...
  const root = {
    ...
    values: TIME_ZONE_OPTIONS.map((timeZone) => ({
      can_select: true,
      description: `(${formatTimeZoneOffset(timeZone, ns)})`,
      name: timeZone,
      timeZone,
    })),
  };
...
export function getTimeZonePath(
  timeZone: string,
  ns: bigint,
): TreePath | undefined {
...
export function getTimeZoneFromPath(
  path: TreePath | null,
  ns: bigint,
): string | null {
 ...
 */
export function formatTimestampCopyText(ns: bigint): string {
 ...
 *
 * Mounted through `PlaybackShell`'s `timelineExtraActions` slot.
 */
const TimestampReadout: React.FC = () => {
  ...
  const handleCopy = () => {
    void navigator.clipboard?.writeText(formatTimestampCopyText(absoluteNs));
    showCopied(true);
  };
...
export default TimestampReadout;
