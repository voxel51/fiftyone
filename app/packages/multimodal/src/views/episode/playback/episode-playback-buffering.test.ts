...
describe("episode playback buffering policy", () => {
  it("derives bounded startup and cache windows from the tick rate", () => {
    const policy = deriveEpisodePlaybackPolicy(
      DEFAULT_EPISODE_PLAYBACK_POLICY,
      10,
    );
  ...
  it("keeps background work idle and user-visible reads foregrounded", () => {
    expect(episodeBatchReadPriority("background-lookahead")).toBe("idle");
    expect(episodeBatchReadPriority("loopback-lookahead")).toBe("playback");
    expect(episodeBatchReadPriority("playback-prefetch")).toBe("playback");
    expect(episodeBatchReadPriority("startup-lookahead")).toBe("playback");
  });
  ...
  it("queues bounded background batches and stops when coverage is complete", () => {
    ...
    const policy = {
      ...deriveEpisodePlaybackPolicy(DEFAULT_EPISODE_PLAYBACK_POLICY, 10),
