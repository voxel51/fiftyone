...
import { useEpisodeTemporalTags } from "./use-episode-temporal-tags";
...
describe("useEpisodeTemporalTags", () => {
  describe("tracks", () => {
    it("returns an empty array when there are no temporal tags", () => {
      const { result } = renderHook(() => useEpisodeTemporalTags(ctx));
    ...
    it("assigns a deterministic color based on the label", () => {
      ...
      const { result: r1 } = renderHook(() => useEpisodeTemporalTags(ctx));
      const { result: r2 } = renderHook(() => useEpisodeTemporalTags(ctx));
