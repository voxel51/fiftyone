...
export interface NetworkHealthEstimatorOptions {
 ...
 */
export function createNetworkHealthEstimator(
  options: NetworkHealthEstimatorOptions = {},
): {
  onTransportSample(sample: LaneTransportSnapshot, atMs: number): void;
  setBuffering(buffering: boolean, atMs: number): void;
  evaluate(atMs: number): NetworkHealth;
} {
 ...
 */
export function shouldPublishNetworkHealth(
...
export interface IdleWorkGateSignals {
...
export function shouldDeferIdleWork(signals: IdleWorkGateSignals): boolean {
  if (!signals.limited) {
    return false;
  }
