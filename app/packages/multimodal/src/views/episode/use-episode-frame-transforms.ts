...
export type EpisodeFrameTransformsStatus = LoadStatus;

/**
 * Resolves a frame transform at a playback time.
 */
export type EpisodeFrameTransformResolver = (
  sourceFrameId: string,
  targetFrameId: string,
  timeNs: bigint,
) => EpisodeFrameTransformResolution;

export type EpisodeFrameGraphSummarizer = (
  dataBearingFrameIds: ReadonlySet<string>,
) => EpisodeFrameGraphSummary;

export type EpisodeFramePlacementReadinessStatus =
  | "ready"
  | "loading"
  | "needsFetch"
  | "definitiveMissing";

export interface EpisodeFramePlacementReadiness {
  readonly frameIds: readonly string[];
  readonly status: EpisodeFramePlacementReadinessStatus;
}

export type EpisodeFramePlacementReadinessGetter = ({
  frameIds,
  targetFrameId,
  timeNs,
}: {
...
export type EpisodeFramePlacementPrefetcher = (timeNs: bigint) => void;

/** Source-scoped transform graph, placement resolver, and load state. */
export interface EpisodeFrameTransformsState {
  readonly error: string | null;
  readonly frameIds: readonly string[];
  readonly getPlacementReadiness: EpisodeFramePlacementReadinessGetter;
  ...
  readonly prefetchPlacement: EpisodeFramePlacementPrefetcher;
  readonly resolve: EpisodeFrameTransformResolver;
  readonly status: EpisodeFrameTransformsStatus;
  readonly summarizeGraph: EpisodeFrameGraphSummarizer;
  /** Changes only when the normalized transform edge inventory changes. */
  readonly topologyRevision?: number;
}

export interface UseEpisodeFrameTransformsOptions {
...
interface EpisodeFrameTransformsInternalState {
  readonly error: string | null;
  readonly status: EpisodeFrameTransformsStatus;
 ...
 */
export function useEpisodeFrameTransforms({
  capability,
  dynamicRange,
  policy,
  sourceKey,
  timeNs,
}: UseEpisodeFrameTransformsOptions): EpisodeFrameTransformsState {
  const storeRef = useRef<EpisodeFrameTransformStore | null>(null);
  const [state, setState] = useState<EpisodeFrameTransformsInternalState>({
  ...
  useEffect(() => {
    ...
    if (
      playbackStore &&
      shouldDeferEpisodeIdleWorkForStore(playbackStore, null)
    ) {
      return;
    }
  ...
  const resolve = useCallback<EpisodeFrameTransformResolver>(
  ...
  const getPlacementReadiness =
    useCallback<EpisodeFramePlacementReadinessGetter>(
      ({
        frameIds: requestedFrameIds,
        targetFrameId,
        timeNs: requestTimeNs,
      }) => {
  ...
  const summarizeGraph = useCallback<EpisodeFrameGraphSummarizer>(
    (dataBearingFrameIds) =>
