...
describe("useEpisodeGridPreview", () => {
  beforeEach(() => {
  ...
  it("plays additional preview frames on hover", async () => {
    const latestState = { current: null as EpisodeGridPreviewState | null };
  ...
  it("reports buffering only when a hover frame read stays pending", async () => {
    ...
    act(() => {
      vi.advanceTimersByTime(EPISODE_GRID_BUFFERING_DELAY_MS - 1);
    });
    expect(latestState.current?.isBuffering).toBe(false);
...
function formatState(state: EpisodeGridPreviewState): string {
  return [
    state.status,
