 ...
 */
export const GRID_STREAM_AUTO = "auto" as const;
...
function normalizeSelectedStream(stream: string | null | undefined) {
  ...
  return normalized ? normalized : GRID_STREAM_AUTO;
}
...
function readStoredSelection(datasetName: string): string {
  return readStorageValue(storageKey(datasetName)) ?? GRID_STREAM_AUTO;
}
 ...
 */
export function useRegisterGridStreams() {
 ...
 */
export function useGridStreams(datasetName?: string) {
  return useStreamSnapshot(datasetName);
}
    ...
    setSelectedStreamByDataset((current) =>
      current.has(datasetName)
        ...
        : updateSelectedStream(
            current,
            datasetName,
            storedSelection ?? GRID_STREAM_AUTO,
          ),
    );
 ...
 * Reads and updates the per-dataset episode grid preview stream override.
 */
export function useGridSelectedStream(datasetName?: string) {
 ...
 */
export function __resetGridStreamStateForTests() {
