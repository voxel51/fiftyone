...
import { checkboxNoSpaceToggleProps } from "./episode-settings-keyboard";
import { matchesEpisodeStreamFilter } from "./episode-stream-filter";
import EpisodePlotTileStyles from "./EpisodePlotTile.module.css";
import { EpisodeSettingsFilterInput } from "./EpisodeSettingsFilterInput";
 ...
 * tile is focused.
 */
const EpisodePlotTileSettings: React.FC = () => {
  const { enumeration } = useEpisodeNumericSeriesContext();
  const seriesConfigs = useEpisodePlotTileSeries();
  const toggleSeries = useToggleEpisodePlotSeries();
...
function PlotStreamGroup({
  ...
  readonly seriesConfigs: readonly EpisodePlotSeriesConfig[];
  ...
  const enabledByPath = useMemo(() => {
    const byPath = new Map<string, EpisodePlotSeriesConfig>();
          ...
          return (
            ...
            >
              ...
              {enabled ? (
                <span
                  aria-hidden="true"
                  className={EpisodePlotTileStyles.swatch}
...
function filterStreams(
  ...
  for (const stream of streams) {
    if (matchesEpisodeStreamFilter(filter, stream.sourceName)) {
      matches.push(stream);
      continue;
    }
    const fields = stream.fields.filter((field) =>
      matchesEpisodeStreamFilter(filter, field.path),
    );
    if (fields.length > 0) {
...
export default EpisodePlotTileSettings;
