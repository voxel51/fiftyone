...
describe("episode camera model", () => {
  it("applies ROS ROI before binning", () => {
    const effective = effectiveEpisodeCameraCalibration(
      calibration({
        binningX: 2,
        binningY: 4,
        height: 600,
  ...
  it("resolves Auto from canonical image geometry suffixes", () => {
    expect(suggestEpisodeImageGeometry("/camera/front/image_raw")).toBe(
      "original",
    );
    expect(
      suggestEpisodeImageGeometry("/camera/front/image_rect/compressed"),
    ).toBe("rectified");
    expect(
      suggestEpisodeImageGeometry("/camera/front/image_raw/compressedDepth"),
    ).toBe("original");
    expect(
      suggestEpisodeImageGeometry("/camera/front/image_raw/compress"),
    ).toBe(null);

    expect(suggestEpisodeImageGeometry("/raw/camera/front/image")).toBeNull();

    const rectified = resolveEpisodeCameraModel({
    ...
    const original = resolveEpisodeCameraModel({
      calibration: calibration({
        D: [-0.2, 0.03, 0, 0, 0],
        distortionModel: "plumb_bob",
        P,
  ...
  it("keeps Auto ambiguous when the image stream has no geometry evidence", () => {
    const resolved = resolveEpisodeCameraModel({
      calibration: calibration({
        D: [-0.2, 0.03, 0, 0, 0],
        distortionModel: "plumb_bob",
        P,
  ...
  it("projects plumb-bob calibration through the rational model", () => {
    ...
    expect(
      projectEpisodeCameraPoint(resolved.model, [0.5, 0, 1])?.u,
    ).toBeCloseTo(101.25);
  });
  ...
  it("culls rational points beyond the monotonic image domain", () => {
    ...
    expect(projectEpisodeCameraPoint(resolved.model, [10, 0, 1])).toBeNull();
  });
  ...
  it("keeps valid behind-camera rays for greater-than-180-degree fisheye", () => {
    ...
    const projected = projectEpisodeCameraPoint(resolved.model, point);
  ...
  it("applies rectification before the rectified projection matrix", () => {
    ...
    expect(projectEpisodeCameraPoint(resolved.model, [1, 0, 10])).toMatchObject(
      {
        u: 50,
        v: 60,
      },
