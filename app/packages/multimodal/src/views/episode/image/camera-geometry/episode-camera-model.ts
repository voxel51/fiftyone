...
const IMAGE_TRANSPORT_STREAM_TOKENS = new Set([
  "compressed",
  "compresseddepth",
  "compressedimage",
]);
...
export interface EpisodeEquidistantCameraModel {
  readonly D: readonly number[];
  readonly height: number;
  readonly K: readonly number[];
...
export type EpisodeCameraModel =
  | EpisodeEquidistantCameraModel
  | EpisodePinholeCameraModel
  | EpisodeRationalCameraModel;

/** Effective calibration after applying ROS binning and sensor-space ROI. */
export interface EpisodeEffectiveCameraCalibration {
  readonly D?: readonly number[];
  readonly K: readonly number[];
...
export type EpisodeCameraModelResolution =
  | {
      readonly equivalentDisplacementPx: number;
      readonly model: EpisodeCameraModel;
      readonly mode: EpisodeResolvedImageGeometryMode;
  ...
  | {
      ...
      readonly suggestedMode: EpisodeResolvedImageGeometryMode | null;
    };

/** One finite camera-space point projected into calibration pixels. */
export interface EpisodeProjectedCameraPoint {
...
type CameraModelBuildResult = EpisodeCameraModel | CameraModelBuildFailure;
 ...
 */
export function effectiveEpisodeCameraCalibration(
  calibration: CameraCalibrationVisualization,
): EpisodeEffectiveCameraCalibration | null {
  if (
...
export function resolveEpisodeCameraModel({
  calibration,
  geometry,
  imageStream,
}: {
...
}): EpisodeCameraModelResolution {
  const effective = effectiveEpisodeCameraCalibration(calibration);
  if (!effective) {
  ...
  const inferredGeometry = suggestEpisodeImageGeometry(imageStream);
  if (inferredGeometry) {
  ...
  if (!isFailure(original) && !isFailure(rectified)) {
    ...
    if (displacement <= episodeEquivalentPixelTolerance(effective)) {
      return {
...
function cachedCameraModels(
  calibration: EpisodeEffectiveCameraCalibration,
): CameraModelPair {
  ...
  const pair = {
    equivalentDisplacementPx:
      !isFailure(original) && !isFailure(rectified)
        ? maxEpisodeModelDisplacement(original, rectified)
        : null,
    original,
    rectified,
  };
...
export function suggestEpisodeImageGeometry(
  imageStream: string,
): EpisodeResolvedImageGeometryMode | null {
...
export function episodeEquivalentPixelTolerance(
  calibration: Pick<EpisodeEffectiveCameraCalibration, "height" | "width">,
): number {
  return Math.min(
    1,
...
export function maxEpisodeModelDisplacement(
  original: EpisodeCameraModel,
  candidate: EpisodeCameraModel,
): number {
  let maximum = 0;
  let sampleCount = 0;
  for (let row = 0; row < GRID_ROWS; row++) {
    ...
    for (let column = 0; column < GRID_COLUMNS; column++) {
      ...
      const ray = unprojectEpisodeCameraPixel(original, u, v);
      if (!ray) {
        return Number.POSITIVE_INFINITY;
      }
      const reference = projectEpisodeCameraPoint(original, ray);
      const projected = projectEpisodeCameraPoint(candidate, ray);
...
export function projectEpisodeCameraPoint(
  model: EpisodeCameraModel,
  point: readonly [number, number, number],
): EpisodeProjectedCameraPoint | null {
...
export function unprojectEpisodeCameraPixel(
  model: EpisodeCameraModel,
  u: number,
  v: number,
...
function readyOrFailure(
  result: CameraModelBuildResult,
  mode: EpisodeResolvedImageGeometryMode,
): EpisodeCameraModelResolution {
  if (isFailure(result)) {
    return {
      equivalentDisplacementPx: null,
