 ...
 */
const EpisodeImageProjectionOverlay = ({
  cameraModel,
  fit,
  imageHeight,
  imageWidth,
  ...
  const [dwellTooltip, setDwellTooltip] =
    useState<Episode3dHoverTooltipState | null>(null);
  const setSharedHover = useSetEpisodeHoverEcho();
  const publishedHoverRef = useRef<EpisodeHoverEcho | null>(null);
    ...
    const pickAt = (clientX: number, clientY: number) => {
      ...
      void picker
        ...
        .then((pick) => {
          ...
          const tooltip = episodeHoveredPointForFrame(
            layer.stream,
            layer.frame,
            pointIndex,
          );
  ...
  return (
    ...
    >
      {dwellTooltip ? <Episode3dHoverTooltip tooltip={dwellTooltip} /> : null}
    </div>
  );
};
...
export default EpisodeImageProjectionOverlay;
