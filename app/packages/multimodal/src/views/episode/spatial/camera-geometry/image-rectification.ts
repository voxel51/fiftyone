...
export interface RectifiedImageDisplay {
  ...
  readonly projectionModel: PinholeCameraModel;
  readonly width: number;
}

const rectificationCache = new WeakMap<
  CameraModel,
  WeakMap<PinholeCameraModel, RectifiedImageDisplay | null>
>();
 ...
 */
export function resolveRectifiedImageDisplay(
  sourceModel: CameraModel,
  rectifiedModel: CameraModel,
): RectifiedImageDisplay | null {
  if (
  ...
  const result: RectifiedImageDisplay | null = textureMesh
    ? {
        height: imageModel.height,
        pixelTransform: (u: number, v: number) => {
          const ray = unprojectCameraPixel(sourceModel, u, v);
          if (!ray) {
            return null;
          }
          const projected = projectCameraPoint(imageModel, ray);
...
function createRectificationTextureMesh(
  sourceModel: CameraModel,
  targetModel: PinholeCameraModel,
  ...
  for (let row = 0; row <= rows; row++) {
    ...
    for (let column = 0; column <= columns; column++) {
      ...
      const ray = unprojectCameraPixel(targetModel, targetU, targetV);
      const source = ray ? projectCameraPoint(sourceModel, ray) : null;
      if (
        !source ||
        source.u < 0 ||
        source.v < 0 ||
...
function rectifiedImageModel(model: PinholeCameraModel): PinholeCameraModel {
