...
export type ImageGeometryMode = "auto" | "original" | "rectified";
...
export interface PinholeCameraModel {
  ...
  readonly space: ResolvedImageGeometryMode;
  readonly width: number;
}

/** OpenCV rational-polynomial projection with a bounded normalized radius. */
export interface RationalCameraModel {
...
export interface EquidistantCameraModel {
  readonly D: readonly number[];
  readonly height: number;
  readonly K: readonly number[];
...
export type CameraModelResolution =
  | {
      readonly equivalentDisplacementPx: number;
      readonly model: CameraModel;
      readonly mode: ResolvedImageGeometryMode;
  ...
  | {
      ...
      readonly suggestedMode: ResolvedImageGeometryMode | null;
    };
...
type CameraModelBuildResult = CameraModel | CameraModelBuildFailure;
 ...
 */
export function effectiveCameraCalibration(
  calibration: CameraCalibrationVisualization,
): EffectiveCameraCalibration | null {
  if (
...
export function resolveCameraModel({
  ...
  if (!isFailure(original) && !isFailure(rectified)) {
    ...
    if (displacement <= equivalentPixelTolerance(effective)) {
      return {
  ...
  const pair = {
    equivalentDisplacementPx:
      !isFailure(original) && !isFailure(rectified)
        ? maxModelDisplacement(original, rectified)
        : null,
    original,
    rectified,
  };
...
function cameraModelCacheKey(calibration: EffectiveCameraCalibration): string {
  return [
    calibration.width,
    calibration.height,
    calibration.distortionModel ?? "",
...
function maxModelDisplacement(
  ...
  for (let row = 0; row < GRID_ROWS; row++) {
    ...
    for (let column = 0; column < GRID_COLUMNS; column++) {
      ...
      const ray = unprojectCameraPixel(original, u, v);
      if (!ray) {
        return Number.POSITIVE_INFINITY;
      }
      const reference = projectCameraPoint(original, ray);
      const projected = projectCameraPoint(candidate, ray);
...
function readyOrFailure(
  result: CameraModelBuildResult,
  mode: ResolvedImageGeometryMode,
): CameraModelResolution {
  if (isFailure(result)) {
    return {
      equivalentDisplacementPx: null,
