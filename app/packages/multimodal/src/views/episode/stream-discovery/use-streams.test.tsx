...
describe("useStreams", () => {
  it("exposes the neutral manifest streams", () => {
    ...
    const state = useStreams({ session, sourceAvailable: true });
  ...
  it("reports idle, loading, and adapter errors without stale inventory", () => {
    expect(useStreams({ session: null, sourceAvailable: false }).status).toBe(
      "idle",
    );
    expect(useStreams({ session: null, sourceAvailable: true }).status).toBe(
      "loading",
    );
    expect(
      useStreams({
