...
import {
  EPISODE_NOTICE_APPEARANCE_FLOOR_MS,
  EPISODE_NOTICE_DISAPPEAR_LINGER_MS,
  buildEpisode3dPlacementNotices,
  buildEpisode3dTransformNotices,
  buildEpisodeCameraTargetNotice,
...
describe("buildEpisode3dPlacementNotices", () => {
  it("returns nothing while no placement is pending", () => {
    expect(buildEpisode3dPlacementNotices(NO_PLACEMENT_IDS)).toEqual([]);
  });

  it("builds the placement-loading quartet with frame ids in detail", () => {
    const notices = buildEpisode3dPlacementNotices({
...
describe("buildEpisode3dTransformNotices", () => {
  it("returns nothing when transforms resolve cleanly", () => {
    expect(buildEpisode3dTransformNotices(NO_TRANSFORM_CONDITIONS)).toEqual([]);
  });

  it("short-circuits everything else on a window fetch failure", () => {
    const notices = buildEpisode3dTransformNotices({
...
describe("buildEpisodeCameraTargetNotice", () => {
  const missingTarget = {
  ...
  it("warns when a follow-mode target transform is missing", () => {
    expect(buildEpisodeCameraTargetNotice(missingTarget)).toEqual({
  ...
  it("stays quiet before both frames are selected", () => {
    expect(
      buildEpisodeCameraTargetNotice({ ...missingTarget, worldFrameId: "" }),
    ).toBeNull();
    expect(
      buildEpisodeCameraTargetNotice({
        ...missingTarget,
...
describe("buildEpisodePointCloudSamplingNotice", () => {
  ...
  it("stays quiet while every cloud renders in full", () => {
    expect(buildEpisodePointCloudSamplingNotice(null, 150_000)).toBeNull();
    expect(
      buildEpisodePointCloudSamplingNotice(
...
describe("buildEpisodeTileStreamNotice", () => {
...
describe("buildEpisodeTileEmptyStateModel", () => {
...
function testNotice(
  id: string,
  overrides: Partial<EpisodeHealthNotice> = {},
): EpisodeHealthNotice {
  return {
    id,
...
describe("createEpisodeNoticeStabilizer", () => {
  ...
  it("returns the same array identity while the visible content holds", () => {
    ...
    clock.advance(EPISODE_NOTICE_APPEARANCE_FLOOR_MS);
  ...
  it("shows a boundary-oscillating condition once and keeps it", () => {
    ...
    expect(firstVisibleAt).toBe(EPISODE_NOTICE_APPEARANCE_FLOOR_MS);
  ...
  it("survives dropouts up to the linger and retires past it", () => {
    ...
    clock.advance(100);
    expect(stabilizer.update([])).toHaveLength(1);
    clock.advance(EPISODE_NOTICE_DISAPPEAR_LINGER_MS - 200);
    expect(stabilizer.update([])).toHaveLength(1);
...
describe("useStabilizedEpisodeNotices", () => {
  afterEach(() => {
    cleanup();
    vi.useRealTimers();
  });
  ...
  it("applies the appearance floor and linger through wall-clock wakes", () => {
    ...
    const { rerender, result } = renderHook(
      ({ notices }: { notices: readonly EpisodeHealthNotice[] }) =>
        useStabilizedEpisodeNotices(notices),
    ...
    act(() => {
      vi.advanceTimersByTime(EPISODE_NOTICE_APPEARANCE_FLOOR_MS + 10);
    });
    ...
    act(() => {
      vi.advanceTimersByTime(EPISODE_NOTICE_DISAPPEAR_LINGER_MS + 10);
    });
    expect(result.current).toEqual([]);
  });
