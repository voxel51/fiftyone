...
/**
 * Network-health verdict for the active episode source, shown in modal chrome so
 ...
 * decode instead.
 */
export interface EpisodeNetworkHealth {
...
export interface EpisodeNetworkHealthEstimatorOptions {
...
interface TransportDelta {
  ...
  readonly lane: LaneTransportSnapshot["lane"];
 ...
 */
export function createEpisodeNetworkHealthEstimator(
  options: EpisodeNetworkHealthEstimatorOptions = {},
): {
  onTransportSample(sample: LaneTransportSnapshot, atMs: number): void;
  setBuffering(buffering: boolean, atMs: number): void;
  evaluate(atMs: number): EpisodeNetworkHealth;
} {
  ...
  const lastSnapshots = new Map<
    LaneTransportSnapshot["lane"],
    LaneTransportSnapshot["snapshot"]
  >();
 ...
 */
export function shouldPublishEpisodeNetworkHealth(
  previous: EpisodeNetworkHealth,
  next: EpisodeNetworkHealth,
): boolean {
...
export interface EpisodeIdleWorkGateSignals {
...
export function shouldDeferEpisodeIdleWork(
  signals: EpisodeIdleWorkGateSignals,
): boolean {
  if (!signals.limited) {
    return false;
  }
