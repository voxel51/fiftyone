...
const Publisher: React.FC<{ readonly sourceId: string }> = ({ sourceId }) => {
  usePublishEpisodeImageTileBinding(sourceId);
  return null;
};

const BindingsProbe: React.FC = () => (
  <span data-testid="bindings">
    {JSON.stringify(useEpisodeImageTileBindings())}
  </span>
);

describe("usePublishEpisodeImageTileBinding", () => {
  afterEach(() => cleanup());
...
describe("useEpisodeImageTileHoverProps", () => {
  it("publishes hover on enter and clears its own hover on leave", () => {
    const store = createStore();
    const { result } = renderHook(
      () => useEpisodeImageTileHoverProps("cam_front"),
      {
    ...
    expect(store.get(episodeHoveredImageStreamAtom)).toBe("cam_front");

    act(() => result.current.onPointerLeave());
    expect(store.get(episodeHoveredImageStreamAtom)).toBeNull();
  });
  ...
  it("does not clear another tile's hover, and cleans up on unmount", () => {
    ...
    store.set(episodeHoveredImageStreamAtom, "cam_back");
    act(() => result.current.onPointerLeave());
    expect(store.get(episodeHoveredImageStreamAtom)).toBe("cam_back");
