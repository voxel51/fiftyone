...
export interface EpisodeBulkStreamControl {
 ...
 * accumulation.
 */
export function startEpisodeBulkStreamLifecycle({
  initialDelayMs,
  retryDelayMs,
  runStream,
  shouldStandDown,
  ...
  readonly runStream: (
    stream: string,
    control: EpisodeBulkStreamControl,
  ) => Promise<void>;
...
export function shouldDeferEpisodeBulkHistory(
  playbackStore: PlaybackStore | null,
): boolean {
  if (!playbackStore) return false;
  if (
    getIsPlaying(playbackStore) &&
    getEpisodeNetworkHealth(playbackStore).limited
  ) {
    return true;
  }
  return shouldDeferEpisodeIdleWorkForStore(playbackStore, null);
}
