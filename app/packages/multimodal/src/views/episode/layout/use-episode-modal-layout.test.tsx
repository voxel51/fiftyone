...
vi.mock("../image/EpisodeImageTile", () => ({
...
describe("useEpisodeModalLayout", () => {
  beforeEach(() => {
    localStorage.clear();
  });
  afterEach(() => cleanup());
  ...
  it("restores and updates camera conventions in the selected media field", () => {
    ...
    expect(readEpisodeCameraPreferences("dataset-a", "mcap")).toMatchObject({
    ...
    expect(
      readEpisodeCameraPreferences("dataset-a", "mcap")?.preferredWorldFrameId,
    ).toBeUndefined();
  });
  ...
  it("falls back to dataset-scoped scene-axis persistence without a media field", () => {
    ...
    expect(readEpisodeModalLayout("dataset-a")?.sceneUpAxis).toBe("y");
  });
  ...
  it("restores the persisted sidebar width", () => {
    writeEpisodeModalLayout({ sidebarWidthPx: 480 }, "dataset-a");
    const { result } = renderLayoutHook(SCENE_SOURCES, "dataset-a");
  ...
  it("restores and persists the dataset scene up-axis", () => {
    writeEpisodeModalLayout({ sceneUpAxis: "y" }, "dataset-a");
    const { result } = renderLayoutHook(SCENE_SOURCES, "dataset-a");
    ...
    expect(readEpisodeModalLayout("dataset-a")?.sceneUpAxis).toBe("x");
    expect(readEpisodeModalLayout("dataset-b")?.sceneUpAxis).toBeUndefined();
  });
...
describe("EpisodeModalLayoutPersistence", () => {
  beforeEach(() => {
    localStorage.clear();
    vi.useFakeTimers();
  });
  ...
  it("writes layout changes after the debounce window", () => {
    ...
    expect(readEpisodeModalLayout()?.layout).toBe("camera-default");
  });
  ...
  it("writes under the dataset it was given", () => {
    render(
      <TilingProvider initialTiles={TWO_TILES}>
        <LayoutDriver next="camera-default" />
        <EpisodeModalLayoutPersistence datasetId="dataset-a" />
      </TilingProvider>,
    );
    ...
    expect(readEpisodeModalLayout("dataset-a")?.layout).toBe("camera-default");
  });
  ...
  it("writes expanded tile changes after the debounce window", () => {
    ...
    expect(readEpisodeModalLayout("dataset-a")?.expandedTileId).toBe(
      "camera-default",
    );
    expect(readEpisodeModalLayout("dataset-a")?.layout).toBeUndefined();
  });
  ...
  it("writes manual title changes after the debounce window", () => {
    ...
    expect(readEpisodeModalLayout("dataset-a")?.tileTitles).toEqual({
  ...
  it("does not persist a layout the user never edited", () => {
    ...
    unmount();
    expect(readEpisodeModalLayout("dataset-a")).toBeNull();
  });
});
