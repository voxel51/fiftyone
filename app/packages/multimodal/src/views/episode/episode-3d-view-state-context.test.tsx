...
import {
  __resetEpisode3dViewStateScopesForTests,
  Episode3dViewStateProvider,
  useEpisode3dViewStateStore,
} from "./episode-3d-view-state-context";
...
describe("Episode3dViewStateProvider inspection scopes", () => {
  ...
  it("retains 33 scopes mounted in one commit before evicting", () => {
    const stores = new Map<
      string,
      ReturnType<typeof useEpisode3dViewStateStore>
    >();
...
function ScopedStoreGroup({
  ...
  readonly stores: Map<string, ReturnType<typeof useEpisode3dViewStateStore>>;
}) {
  return (
    <>
      {Array.from({ length: count }, (_, index) => {
        ...
        return (
          <Episode3dViewStateProvider key={scopeKey} scopeKey={scopeKey}>
...
function ReconciledStoreHarness({
  ...
  return (
    <>
      {includePrimary ? (
        <Episode3dViewStateProvider scopeKey="shared">
          <StoreCapture captureKey="primary" stores={stores} />
        </Episode3dViewStateProvider>
      ) : null}
      <Episode3dViewStateProvider scopeKey={subjectScope}>
...
function StoreCapture({
  ...
  const store = useEpisode3dViewStateStore();
...
function renderScopedStore(scopeKey: string) {
  return renderHook(useEpisode3dViewStateStore, {
    wrapper: ({ children }: PropsWithChildren) => (
      <Episode3dViewStateProvider scopeKey={scopeKey}>
        {children}
      </Episode3dViewStateProvider>
    ),
  });
