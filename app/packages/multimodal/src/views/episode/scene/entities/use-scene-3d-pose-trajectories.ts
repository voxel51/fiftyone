...
import { usePoseTrajectoriesContext } from "./pose-trajectories-context";
import {
 ...
 */
export function useScene3dPoseTrajectories({
  annotationFrames,
  frameIds,
  playbackTimeNs,
  poseFrames,
  ...
  const trajectories = usePoseTrajectoriesContext();
  ...
  const syntheticPoseAnnotations = useMemo(() => {
    ...
    const playbackFrames: StreamPlaybackFrame<SceneUpdateVisualization>[] = [];
