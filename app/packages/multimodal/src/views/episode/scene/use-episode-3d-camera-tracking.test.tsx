...
type TrackingProps = Parameters<typeof useEpisode3dCameraTracking>[0];

describe("useEpisode3dCameraTracking", () => {
  it("defaults to follow position with no pose command", () => {
    const { result } = renderHook(useEpisode3dCameraTracking, {
  ...
  it("runs effectively free when automatic targeting has only the reference frame", () => {
    const { rerender, result } = renderHook(useEpisode3dCameraTracking, {
      initialProps: trackingProps({
  ...
  it("records the final pose and composition on unmount for mid-gesture hops", () => {
    const anchor = trackingAnchor({});
    const { result, unmount } = renderHook(useEpisode3dCameraTracking, {
      initialProps: trackingProps({ placementStatus: "transformed" }),
    });
...
describe("useEpisode3dCameraTracking world-frame changes", () => {
...
describe("useEpisode3dCameraTracking view-state restore", () => {
...
function trackingAnchor(
  overrides: Partial<Episode3dCameraTrackingAnchor>,
