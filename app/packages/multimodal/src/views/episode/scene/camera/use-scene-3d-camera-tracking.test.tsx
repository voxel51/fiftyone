...
import type { Scene3dCameraTrackingAnchor } from "./scene-3d-camera";
import {
...
} from "./use-scene-3d-camera-tracking";
...
type TrackingProps = Parameters<typeof useScene3dCameraTracking>[0];

describe("useScene3dCameraTracking", () => {
  ...
  it("records the final pose and composition on unmount for mid-gesture hops", () => {
    const anchor = trackingAnchor({});
    const { result, unmount } = renderHook(useScene3dCameraTracking, {
      initialProps: trackingProps({ placementStatus: "transformed" }),
    });
  ...
  it("drops provisional remap memory when the selected streams change", () => {
    const { rerender, result } = renderHook(useScene3dCameraTracking, {
      initialProps: trackingProps({
        frameTransforms: missingTransforms(),
        placementStatus: "empty",
    ...
    rerender(
      trackingProps({
        ...
        provisionalFrameIds: ["lidar"],
        provisionalPlaybackFrame: provisionalFrame("lidar", 5n),
        selectedStreamsKey: "other-streams",
      }),
    );
    rerender(
      trackingProps({
...
describe("useScene3dCameraTracking world-frame changes", () => {
...
describe("useScene3dCameraTracking view-state restore", () => {
...
function trackingAnchor(
  overrides: Partial<Scene3dCameraTrackingAnchor>,
...
function missingTransforms(): FrameTransformsState {
  return {
    error: null,
    frameIds: [],
