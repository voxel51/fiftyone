...
import type { EpisodeReferenceTransition } from "../../spatial/frame-transforms/reference-selection";
...
export type Episode3dPlacementStatus =
  | "empty"
  | "provisional"
  | "transformed"
  | "unframed";
...
export type { CameraTargetResolution } from "./episode-3d-camera";

/** Camera intent captured at a 3D source epoch and carried into the next one. */
export interface Episode3dCameraTrackingRestore {
 ...
 * gestures and the Episode3dCameraRig composes follow modes per target update,
 ...
 */
export function useEpisode3dCameraTracking({
  cameraNavigationMode = DEFAULT_EPISODE_3D_CAMERA_NAVIGATION_MODE,
  cameraTargetFrameId,
  cameraTargetSelectionSource,
  defaultTrackingMode = DEFAULT_EPISODE_3D_TRACKING_MODE,
  frameTransforms,
  placementStatus,
  playbackTimeNs,
  provisionalFrameIds,
  ...
  readonly provisionalPlaybackFrame: EpisodeStreamPlaybackFrame<PointCloudVisualization> | null;
  ...
  readonly onDefaultTrackingModeChange?: (mode: Episode3dTrackingMode) => void;
  ...
  readonly restore?: Episode3dCameraTrackingRestore | null;
  ...
  readonly worldFrameTransition?: EpisodeReferenceTransition | null;
  ...
  const [trackingMode, setTrackingMode] =
    useState<Episode3dTrackingMode>(restoredTrackingMode);
  ...
  const [adoptAnchor, setAdoptAnchor] =
    useState<Episode3dCameraTrackingAnchor | null>(null);
  const latestCameraPoseRef = useRef<PointCloudCameraPose | null>(null);
  const latestAnchorRef = useRef<Episode3dCameraTrackingAnchor | null>(null);
  ...
  const pendingCameraViewRestoreRef =
    useRef<Episode3dCameraViewSnapshot | null>(
      restore?.cameraView &&
  ...
  const applyPendingComposition = useCallback(() => {
    ...
    let resolved: Extract<
      ReturnType<typeof resolveEpisode3dCameraComposition>,
      { status: "resolved" }
    > | null = null;
    let resolvedComposition: Episode3dCameraComposition | null = null;
    for (const composition of compositions) {
      ...
      const carriedTargetResolution = resolveAgainstCarriedTarget
        ...
        : cameraTargetResolution;
      const candidate = resolveEpisode3dCameraComposition({
  ...
  const onPoseSample = useCallback(
    (sample: Episode3dCameraRigSample) => {
