...
import {
  ...
  useEpisodeMapViewportScope,
  writeEpisodeMapViewport,
} from "./episode-map-viewport-cache";
import { episodeMapViewportIsNearEvidence } from "./episode-map-viewport-proximity";
import { useEpisodeDataStream } from "../playback/episode-data-stream-context";
import { EpisodeMapPlaybackController } from "./episode-map-playback-controller";
import {
...
import styles from "./EpisodeMapTile.module.css";

const HIT_SOURCE_ID = "episode-location-hit-points";
const CURRENT_SOURCE_ID = "episode-location-current";
const HOVER_SOURCE_ID = "episode-location-hover";
const MEASURE_LINE_SOURCE_ID = "episode-location-measure-line";
const MEASURE_PREVIEW_SOURCE_ID = "episode-location-measure-preview";
const MEASURE_POINTS_SOURCE_ID = "episode-location-measure-points";

const HIT_LAYER_ID = "episode-location-hit-points";
const ACCURACY_LAYER_ID = "episode-location-accuracy";
const PULSE_LAYER_ID = "episode-location-pulse";
...
const EpisodeMapTile: React.FC<EpisodeTileProps> = () => {
  ...
  const mapViewportScope = useEpisodeMapViewportScope();
  ...
  const [basemapState, setBasemapState] = useState<{
    ...
    status: initialEpisodeMapBasemapStatus(settings.baseLayer),
  }));
  ...
  const basemapStatus =
    basemapState.baseLayer === settings.baseLayer
      ? basemapState.status
      : initialEpisodeMapBasemapStatus(settings.baseLayer);
...
function addEpisodeMapSourcesAndLayers(map: MapLibreMap) {
...
function mapTileTitle(
  ...
  if (locationStreamCount > 1) {
    return `Map (${locationStreamCount})`;
  }
  return "Map";
}
...
function mapStatusText({
  ...
  if (locationStreamCount === 0 || enabledStreamCount === 0) {
    return null;
  }
  const notes = [
