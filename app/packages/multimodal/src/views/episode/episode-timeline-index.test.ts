...
import {
  ...
  type McapTimelineRange as EpisodeTimelineRange,
...
describe("EpisodeTimelineIndex", () => {
  it("represents the default 30 Hz tick grid without materializing it", () => {
    const index = createEpisodeTimelineIndex(timelineRange(100_000_000n));
  ...
  it("keeps long recordings addressable past the old 20k tick cap", () => {
    const index = createEpisodeTimelineIndex(timelineRange(7_200_000_000_000n));
  ...
  it("rejects ranges whose end precedes their start", () => {
    expect(() => createEpisodeTimelineIndex(timelineRange(1n, 2n))).toThrow(
      "episode timeline range end cannot be before start",
    );
  });
});
