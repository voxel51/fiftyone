...
// registration lifecycle have focused tests in WebGpuViewStage.test.tsx.
vi.mock("../../../visualization/webgpu/WebGpuViewStage", async () => {
...
describe("PlaybackShell shell", () => {
  ...
  it("defaults the playback engine to duration mode", () => {
    ...
    render(
      <PlaybackShell fileName="session.fo">
        <ModeReadout />
      </PlaybackShell>,
    );
  ...
  it("forwards the mode prop through to the playback engine", () => {
    const ModeReadout = () => <span data-testid="mode">{useMode().kind}</span>;
    render(
      <PlaybackShell fileName="session.fo" mode={{ kind: "sequence", fps: 12 }}>
        <ModeReadout />
      </PlaybackShell>,
    );
  ...
  it("renders the default sidebars (settings + inspector empty states)", () => {
    render(<PlaybackShell fileName="x" />);
  ...
  it("removes the right sidebar and its toggle when rightSidebar is null", () => {
    render(<PlaybackShell fileName="x" rightSidebar={null} />);
