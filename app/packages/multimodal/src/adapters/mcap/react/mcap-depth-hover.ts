...
export interface McapDepthSample {
  readonly depthMeters: number;
  readonly pixel: readonly [number, number];
  readonly position: readonly [number, number, number];
}

/** Depth-image point currently under the pointer in an MCAP image tile. */
export interface McapDepthHover extends McapDepthSample {
...
export function useMcapDepthHover(): McapDepthHover | null {
  return useAtomValue(mcapDepthHoverAtom);
}

/** Returns the setter for the modal-local depth hover. */
export function useSetMcapDepthHover() {
  return useSetAtom(mcapDepthHoverAtom);
}
 ...
 */
export function mcapDepthSampleAtDisplayPixel({
  displayCameraModel,
  frame,
  sourceCameraModel,
  u,
  ...
  readonly displayCameraModel: McapCameraModel;
  readonly frame: RawImageVisualization;
  readonly sourceCameraModel: McapCameraModel;
  readonly u: number;
  readonly v: number;
}): McapDepthSample | null {
  if (
    !frame.depth ||
  ...
  const ray = unprojectMcapCameraPixel(sourceCameraModel, x, y);
...
function displayPixelInSourceModel(
  displayCameraModel: McapCameraModel,
  sourceCameraModel: McapCameraModel,
  u: number,
  v: number,
  ...
  const ray = unprojectMcapCameraPixel(displayCameraModel, u, v);
  if (!ray) {
    return null;
  }
  const projected = projectMcapCameraPoint(sourceCameraModel, ray);
