...
import {
  mcapBankingEndOffset,
  mcapBankingStartOffset,
  runMcapByteBankingPass,
} from "./mcap-paused-byte-banking";
 ...
 */
export const McapPausedByteBanking: React.FC<{
  ...
  useEffect(() => {
    ...
    const shouldStandDown = (): boolean =>
      ...
      shouldDeferMcapIdleWorkForStore(store, null);
    ...
    const attempt = async () => {
      ...
      try {
        ...
        const range = await client.readTimelineRange({
        ...
        const endOffset = mcapBankingEndOffset(byteTimeline);
        const playheadTimeNs =
          range.startTimeNs +
          BigInt(Math.round(getPlayhead(store) * 1_000_000_000));

        const outcome = await runMcapByteBankingPass({
          ...
          bytes,
          endOffset,
          fromOffset: mcapBankingStartOffset(byteTimeline, playheadTimeNs),
  ...
  }, [client, remote, source, sourceKey, store, zonedBlockSize]);
