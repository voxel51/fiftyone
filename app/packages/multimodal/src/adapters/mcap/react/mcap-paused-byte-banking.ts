 ...
 * full-history scan, which recordings without a pose topic never get.
 ...
 */
export function mcapBankingStartOffset(
  byteTimeline: readonly McapByteTimelinePoint[],
  timeNs: bigint,
): bigint {
 ...
 */
export function mcapBankingEndOffset(
  byteTimeline: readonly McapByteTimelinePoint[],
): bigint {
...
export interface McapByteBankingPassOptions {
  /**
 ...
 */
export async function runMcapByteBankingPass({
  blockSizeBytesFor,
  bytes,
  endOffset,
  fromOffset,
  ...
  shouldStop,
  source,
}: McapByteBankingPassOptions): Promise<"completed" | "failed" | "stopped"> {
  let bankedBytes = 0n;
