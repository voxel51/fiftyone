...
describe("mcapHoveredPointForFrame", () => {
  it("snapshots position, frame, and every scalar channel at the index", () => {
    const hovered = mcapHoveredPointForFrame("/lidar", pointCloudFrame(), 1);
  ...
  it("omits the frame id when the message carries none", () => {
    const hovered = mcapHoveredPointForFrame(
      "/lidar",
  ...
  it("rejects out-of-range and non-finite picks", () => {
    expect(mcapHoveredPointForFrame("/lidar", pointCloudFrame(), 2)).toBeNull();
    expect(
      mcapHoveredPointForFrame("/lidar", pointCloudFrame(), -1),
    ).toBeNull();
    expect(
      mcapHoveredPointForFrame(
        "/lidar",
        pointCloudFrame({
