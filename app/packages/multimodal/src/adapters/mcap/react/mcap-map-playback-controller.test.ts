...
import {
  McapMapPlaybackController,
  type McapMapPlaybackScheduler,
} from "./mcap-map-playback-controller";

class ManualScheduler implements McapMapPlaybackScheduler {
  private frameId = 0;
...
describe("McapMapPlaybackController", () => {
  ...
  it("contains paint failures and recovers on the next update", () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      "Failed to paint the MCAP map playback frame",
      failure,
    );
