...
export interface McapMapPlaybackScheduler {
...
export interface McapMapPlaybackControllerOptions {
  readonly maxUpdatesPerSecond?: number;
  readonly onPaint: (playheadNs: bigint | null, nowMs: number) => void;
  readonly scheduler?: McapMapPlaybackScheduler;
}
 ...
 */
export class McapMapPlaybackController {
  private active = true;
  ...
  private readonly scheduler: McapMapPlaybackScheduler;
  private timer: ReturnType<typeof setTimeout> | null = null;

  constructor(options: McapMapPlaybackControllerOptions) {
  ...
  private paint(nowMs: number): void {
    ...
    try {
      ...
      console.error("Failed to paint the MCAP map playback frame", error);
    } finally {
...
function browserPlaybackScheduler(): McapMapPlaybackScheduler {
  return {
