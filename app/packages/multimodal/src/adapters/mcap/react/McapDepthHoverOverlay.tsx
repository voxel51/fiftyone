...
import {
  ...
  useSetMcapDepthHover,
  type McapDepthHover,
} from "./mcap-depth-hover";
 ...
 */
export default function McapDepthHoverOverlay(inputs: DepthHoverInputs) {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const inputsRef = useRef(inputs);
  inputsRef.current = inputs;
  const lastPointerRef = useRef<PointerPosition | null>(null);
  const animationFrameRef = useRef<number | null>(null);
  const publishedRef = useRef<McapDepthHover | null>(null);
  const setDepthHover = useSetMcapDepthHover();
  ...
  return <div data-mcap-depth-hover-overlay ref={containerRef} style={style} />;
}
