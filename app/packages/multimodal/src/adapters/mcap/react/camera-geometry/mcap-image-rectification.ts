...
export type McapImagePixelTransform = (
  u: number,
  v: number,
) => readonly [number, number] | null;

/** Cached mapping from recorded image pixels into a rectified display. */
export interface McapRectifiedImageDisplay {
  readonly height: number;
  readonly pixelTransform: McapImagePixelTransform;
  readonly textureMesh: ImageTextureMesh;
  /** Full R/P model used for geometry projected onto the rectified display. */
  readonly projectionModel: McapPinholeCameraModel;
  readonly width: number;
}

const rectificationCache = new WeakMap<
  McapCameraModel,
  WeakMap<McapPinholeCameraModel, McapRectifiedImageDisplay | null>
>();
 ...
 */
export function mcapRectifiedImageDisplay(
  sourceModel: McapCameraModel,
  rectifiedModel: McapCameraModel,
): McapRectifiedImageDisplay | null {
  if (
  ...
  const result: McapRectifiedImageDisplay | null = textureMesh
    ? {
        height: imageModel.height,
        pixelTransform: (u: number, v: number) => {
          const ray = unprojectMcapCameraPixel(sourceModel, u, v);
          if (!ray) {
            return null;
          }
          const projected = projectMcapCameraPoint(imageModel, ray);
...
function createRectificationTextureMesh(
  sourceModel: McapCameraModel,
  targetModel: McapPinholeCameraModel,
  ...
  for (let row = 0; row <= rows; row++) {
    ...
    for (let column = 0; column <= columns; column++) {
      ...
      const ray = unprojectMcapCameraPixel(targetModel, targetU, targetV);
      const source = ray ? projectMcapCameraPoint(sourceModel, ray) : null;
      if (
        !source ||
        source.u < 0 ||
        source.v < 0 ||
