...
import type { McapSourcePlaybackProps } from "../McapSourcePlayback";
import AnyMcapViewer from "./AnyMcapViewer";

const viewerHarness = vi.hoisted(() => ({
  lastPlaybackProps: null as McapSourcePlaybackProps | null,
  reset() {
    this.lastPlaybackProps = null;
  },
  useMcapResourceClient: vi.fn(() => ({ dispose: vi.fn() })),
}));

vi.mock("../McapSourcePlayback", () => ({
  McapSourcePlayback: (props: McapSourcePlaybackProps) => {
...
vi.mock("../use-mcap-resource-client", () => ({
  useMcapResourceClient: viewerHarness.useMcapResourceClient,
}));

describe("AnyMcapViewer", () => {
  beforeEach(() => {
    viewerHarness.reset();
    viewerHarness.useMcapResourceClient.mockClear();
  });
  ...
  it("opens a direct HTTP(S) MCAP URL", () => {
    ...
    expect(viewerHarness.useMcapResourceClient).toHaveBeenLastCalledWith({
      worker: true,
    });
