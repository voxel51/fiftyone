...
/**
 * Cross-tile object selection for the MCAP modal. One object at a time,
 ...
 * carry no object ids, so label + topic matching is the only
...
export type McapSelectionScope = "instance" | "label";

export interface McapSelectedSceneObject {
  readonly kind: "scene-annotation";
  readonly scope: McapSelectionScope;
...
export interface McapSelectedImageObject {
...
export type McapSelectedObject =
  | McapSelectedSceneObject
  | McapSelectedImageObject;

export const mcapSelectedObjectAtom = atom<McapSelectedObject | null>(null);

export function useMcapSelectedObject(): McapSelectedObject | null {
  return useAtomValue(mcapSelectedObjectAtom);
}
...
export function mcapEntityLabel(
...
export function isMcapSceneEntitySelected(
  selected: McapSelectedObject | null,
  topic: string,
  entityId: string,
): boolean {
  return (
 ...
 */
export function isMcapLabelEcho(
  selected: McapSelectedObject | null,
  label: string | null,
): boolean {
  return (
 ...
 */
export function useMcapClearSelectionOnEscape(): void {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (store.get(mcapSelectedObjectAtom) === null) return;
      event.preventDefault();
      event.stopImmediatePropagation();
      store.set(mcapSelectedObjectAtom, null);
    };
 ...
 */
export const McapSelectionHotkeys: React.FC = () => {
  useMcapClearSelectionOnEscape();
  return null;
};
