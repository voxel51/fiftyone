...
import {
  ...
  useMcapVideoDecodeRunway,
} from "./use-mcap-video-decode-runways";
...
describe("h264RunwayFromMessages", () => {
  ...
  it("returns an empty runway while a selected topic has no frame", () => {
    const { result } = renderHook(() =>
      useMcapVideoDecodeRunway("/camera/image", null),
    );
...
function message(frame: EncodedVideoVisualization): McapDecodedMessage {
  return {
    decoded: { output: { visualization: frame } },
  } as McapDecodedMessage;
}
