...
describe("mcap depth ray", () => {
  it("uses the matching displayed frustum pose", () => {
    ...
    const resolveFrameTransform = vi.fn<McapFrameTransformResolver>();
  ...
  it.each(["pending", "missing"] as const)(
    ...
    (status) => {
      const resolveFrameTransform: McapFrameTransformResolver = (
        sourceFrameId,
        targetFrameId,
  ...
  it("uses a resolved fallback transform at the current playhead", () => {
    ...
    const resolution = resolveMcapDepthRay({
      frustumLayers: [],
      hover: depthHover(),
      resolveFrameTransform,
      timeNs: 50n,
...
function depthHover(overrides: Partial<McapDepthHover> = {}): McapDepthHover {
  return {
