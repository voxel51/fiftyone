...
function uniformByteTimeline(
  ...
  const points: McapByteTimelinePoint[] = [];
...
function compute(
  overrides: Partial<Parameters<typeof computeMcapStartupCushion>[0]>,
) {
...
describe("computeMcapStartupCushion", () => {
  ...
  it("caps the cushion at the content ceiling", () => {
    ...
    // the topic caches cannot hold that much decoded lookahead. The wait
