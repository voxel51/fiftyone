...
// before the first chunk, like a real MCAP).
const BYTE_TIMELINE: McapByteTimelinePoint[] = [
  {
...
describe("mcapBankingStartOffset", () => {
  it("starts at the first chunk still ahead of the playhead", () => {
    expect(mcapBankingStartOffset(BYTE_TIMELINE, 0n)).toBe(1_000n);
    expect(mcapBankingStartOffset(BYTE_TIMELINE, 1n * SECOND_NS)).toBe(1_110n);
    expect(mcapBankingStartOffset(BYTE_TIMELINE, 1_500_000_000n)).toBe(1_110n);
  });

  it("falls back to the final chunk past the end of the recording", () => {
    expect(mcapBankingStartOffset(BYTE_TIMELINE, 9n * SECOND_NS)).toBe(1_220n);
    expect(mcapBankingStartOffset([], 0n)).toBe(0n);
  });
});

describe("mcapBankingEndOffset", () => {
  it("ends after the final chunk's own compressed length", () => {
    // Final chunk: starts at 1220, length 300-200=100 → ends at 1320.
    expect(mcapBankingEndOffset(BYTE_TIMELINE)).toBe(1_320n);
    expect(mcapBankingEndOffset([])).toBe(0n);
  });
});

describe("runMcapByteBankingPass", () => {
