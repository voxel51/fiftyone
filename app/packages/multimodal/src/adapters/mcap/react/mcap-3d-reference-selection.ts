import type { McapComposedFrameTransform } from "../frame-transform-types";
...
export interface McapReferenceDecision {
...
export interface McapReferencePromotion {
...
export interface McapReferenceTransition {
  readonly key: string;
  readonly sourceFrameId: string;
...
export interface McapReferenceSelectionState {
  readonly blockedPromotionKey: string | null;
  readonly committedTransition: McapReferenceTransition | null;
  readonly decision: McapReferenceDecision;
  readonly facts: McapReferenceFacts;
  readonly pendingPromotion: McapReferencePromotion | null;
...
export type McapReferenceSelectionAction =
  | {
      readonly facts: McapReferenceFacts;
      readonly timeNs?: bigint;
...
const EMPTY_DECISION: McapReferenceDecision = {
...
export function createMcapReferenceSelectionState(
  facts: McapReferenceFacts,
): McapReferenceSelectionState {
  return {
    blockedPromotionKey: null,
    committedTransition: null,
    decision: deriveMcapReferenceDecision(facts),
    facts,
...
export function mcapReferenceSelectionReducer(
  state: McapReferenceSelectionState,
  action: McapReferenceSelectionAction,
): McapReferenceSelectionState {
  switch (action.type) {
    case "userReferenceSelected": {
      const decision = deriveMcapReferenceDecision(state.facts, action.frameId);
    ...
    case "useRecommendedReference":
      return {
        ...
        blockedPromotionKey: null,
        committedTransition: null,
        decision: deriveMcapReferenceDecision(state.facts),
        pendingPromotion: null,
        userReferenceFrameId: null,
      };
    ...
    case "promotionResolved": {
      ...
      const recommended = deriveMcapReferenceDecision(state.facts);
      if (
    ...
    case "factsChanged": {
      ...
      const recommended = deriveMcapReferenceDecision(
        action.facts,
...
export function deriveMcapReferenceDecision(
  facts: McapReferenceFacts,
  userReferenceFrameId?: string,
): McapReferenceDecision {
...
export function chooseMcapCameraTarget(
...
function pendingPromotionFor({
  ...
  readonly current: McapReferenceDecision;
  readonly facts: McapReferenceFacts;
  readonly recommended: McapReferenceDecision;
  readonly timeNs?: bigint;
}): McapReferencePromotion | null {
  if (
...
function keepLocalReference(
  recommended: McapReferenceDecision,
  current: McapReferenceDecision,
): McapReferenceDecision {
  return {
    ...recommended,
