...
/**
 * Classification-style overlay for `TemporalDetection` labels. Renders
 * a "label confidence" chip in the canvas's top-right corner only when
 * the current playback frame is within the label's `support` range.
 *
 ...
 * inactive.
 */
export class TemporalOverlay
  extends BaseOverlay<TemporalLabel>
  implements Selectable
{
  ...
  public override get label(): TemporalLabel {
    return super.label;
  }

  public override set label(value: TemporalLabel) {
    super.label = value;
