...
import { COMMON_SHORTCUTS, AUDIO_SHORTCUTS } from "../elements/common";
...
let lookerWithReader: AudioLooker | null = null;


export class AudioLooker extends AbstractLooker<AudioState, AudioSample> {
  ...
  get waiting() {
    const audio = this.lookerElement.children[0].element as HTMLAudioElement;
    return (
      audio &&
      (audio.seeking ||
        audio.readyState < 2 ||
  ...
  dispatchImpliedEvents(
    previousState: Readonly<AudioState>,
  ...
  loadOverlays(sample: AudioSample) {
  ...
  pluckOverlays(state: AudioState) {
  ...
  updateSample(sample: AudioSample) {
    this.state.buffers = [[1, 1]];
    this.frames.clear();
