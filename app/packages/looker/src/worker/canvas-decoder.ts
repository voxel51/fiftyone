...
const getPngHeader = async (
  ...
  // Read first 27 bytes: 8 (signature) + 4 (length) + 4 (type) + 13 (IHDR data)
  const header = new Uint8Array(await blob.slice(0, 27).arrayBuffer());
...
export const decodeWithCanvas = async (blob: Blob, cls: string) => {
  ...
  if (blob.type === "image/png") {
    const { bitDepth, colorType } = await getPngHeader(blob);
