...
console.log("Checking types for view-bar");

const localDiagnosticPattern =
  /^(packages[\\/]view-bar|node_modules[\\/]@fiftyone[\\/]view-bar)[\\/]/;
...
try {
  execFileSync(
    ...
    [
      ...
      "--noEmit",
      "-p",
      "packages/view-bar/tsconfig.json",
      "--pretty",
      "false",
    ],
    {
...
const localDiagnostics = lines
  ...
  // fail diagnostics owned by the view-bar package boundary.
...
if (output) {
  console.log(
    "No type issues in packages/view-bar 🚀. TypeScript reported diagnostics outside packages/view-bar; ignoring them for this package-local check.",
  );
}
