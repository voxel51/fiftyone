...
// Typed to the base so either backing stream (the `/frames` DynamicGroupImageStream
...
/**
 * Active dynamic group image stream (decoded per-frame bitmaps). Published so
 ...
 * {@link usePublishDynamicGroupImageStream} so external code can't write arbitrary
 * values; `null` until the dynamic group registrar mounts (e.g. the native-video
 * surface, which has no image stream).
 */
export const useDynamicGroupImageStream = useStream;

/**
 * Publishes `stream` as the active dynamic group image stream for the lifetime of
 * the calling component, clearing on unmount. Intended for the dynamic group image
 * registrar; nothing else should be publishing.
 */
export const usePublishDynamicGroupImageStream = usePublishStream;
