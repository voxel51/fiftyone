...
[`state/accessors.ts`](src/state/accessors.ts) (the _one_ place foreign
recoil/jotai atoms are read), and [`persistence/`](src/persistence) (the delta
supplier from the writeback flow above).
...
flowchart TB
    ...
    HR[handler registration<br/>commands · keybindings · autoInterpolate · deltaSupplier]
...
| `hooks/`           | `useLighterTileScene`, `useVideoAnnotationSyncBundle`, `useWarmupThenSeek`, `useLinkedOverlayState`, `useTemporalOverlayVersion`, `useVfcClockSource`, `useAutoInterpolate`, `useRegisterVideoAnnotation{CommandHandlers,EventHandlers,Keybindings}`, `useVideoAnnotationActions`, `videoCommandHelpers` | Scene lifecycle, sync orchestration, behavior registrars.                                   |
...
| `propagation/`     | `propagationTarget`, `useApplyPropagationResult`                                                                                                                                                                                                                                                         | Resolve a propagation run; write SAM2/linear results to the cache.                          |
| `persistence/`     | `useVideoLabelsDeltaSupplier`                                                                                                                                                                                                                                                                            | Build `/frames/<n>/<field>` JSON-Patch deltas; register via the inversion seam.             |
