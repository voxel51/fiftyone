...
import {
  datasetFragment,
  frameFieldsFragment,
  type frameFieldsFragment$key,
  graphQLSyncFragmentAtomFamily,
  readFragment,
  sampleFieldsFragment,
  type sampleFieldsFragment$key,
  setSidebarGroups,
  type setSidebarGroupsMutation,
  sidebarGroupsFragment,
  type sidebarGroupsFragment$key,
} from "@fiftyone/relay";
import type {
  Field,
  Primitive,
...
export const useGridEntries = (): [
  ...
  const [entries, setEntries] = useReverbStateLoadable(
    sidebarEntries({ modal: false, loading: false, filtered: true }),
  );
  const loadingEntries = useReverbValueLoadable(
