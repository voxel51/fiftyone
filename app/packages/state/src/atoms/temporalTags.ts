 ...
 * `resultsAtom` (a ReverbValue, mirroring `labelTagsCount`). Populate it via
...
export const useSyncTemporalTagResults = (): void => {
  const currentDatasetId = useReverbValue(datasetId);
  const setResults = useSetReverbState(temporalTagResultsAtom);
  const modalActive = useReverbValue(isModalActive);
...
export const useTemporalTagsFieldActive = (): boolean =>
  useReverbValue(activeField({ modal: false, path: TEMPORAL_TAGS_FIELD }));
...
export const useTemporalTagValues = (): string[] => {
  const { results } = useReverbValue(temporalTagResults);
  return useMemo(() => {
