...
/**
 * The set/reset surface a participant receives: the one the core selection
 * atoms are written through, whether a reset's transaction or a publish's
 * callback (see {@link ./useResetExtendedSelection}), so a participant's
 * writes land in one commit with them. Atoms are untyped here: participants
 * own atoms this package has never heard of.
 */
 ...
 * commit. Returns the unregister, for HMR disposal.
 */
