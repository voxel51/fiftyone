...
const useMediaInfo = (
  ...
  const filepath = filepathOverride || sample.sample.filepath;
  ...
  return useMemo(() => {
    ...
    const fps = finitePositive(frameRate ?? sample.frameRate);
    const codec =
...
export const AnnotationTopBar: React.FC<{
  sample: AnnotationTopBarSample;
  /** Overrides the sample's filepath (e.g. per-frame ImaVid filenames). */
  filepathOverride?: string;
  /** Overrides `sample.frameRate` when fps comes from elsewhere. */
  frameRate?: number;
}> = ({ sample, filepathOverride, frameRate }) => {
  const info = useMediaInfo(sample, filepathOverride, frameRate);
