...
export const useLighterDeltaSupplier = (): DeltaSupplier => {
  ...
  const { consumeDeletionDeltas } = usePendingDeletions();
  ...
  return useCallback(() => {
    ...
    // Include deletion deltas for labels removed via undo.
    const currentOverlayIds = new Set(
      scene?.getAllOverlays()?.map((o) => o.id) ?? []
    );
    sampleDeltas.push(...consumeDeletionDeltas(currentOverlayIds));

    return sampleDeltas;
  }, [getLabelDelta, consumeDeletionDeltas, scene]);
};
