...
export enum OperatorLoadResult {
  SUCCESS = "SUCCESS",
  NOT_FOUND = "NOT_FOUND",
  /** Unresolved only because the plugin runtime is still initializing. */
  PENDING = "PENDING",
}
...
export function useOperatorExecutor(
  ...
  // re-resolves once the runtime registers operators, so an executor mounted
  // during initialization picks up its operator and its render-time hooks
  ...
  try {
    ...
    operator = {};
    loadResult = runtimeReady
      ? OperatorLoadResult.NOT_FOUND
      : OperatorLoadResult.PENDING;
  }
  ...
  const pendingInvocations = useRef<
    { paramOverrides: unknown; options?: OperatorExecutorOptions }[]
  >([]);

  const failResolution = useCallback(() => {
  ...
  const execute = useRecoilCallback(
    (state) => async (paramOverrides, options?: OperatorExecutorOptions) => {
      // hold for the replay effect below rather than resolving against a
      // registry that cannot yet supply the operator or its hooks
      if (loadResult === OperatorLoadResult.PENDING) {
        pendingInvocations.current.push({ paramOverrides, options });
        setIsExecuting(true);
        return;
      }
  ...
  // replay invocations made while the runtime was initializing, now that
  // `execute` closes over the resolved operator
  useEffect(() => {
    const pending = pendingInvocations.current;
    if (!pending.length || loadResult === OperatorLoadResult.PENDING) return;
    ...
    for (const { paramOverrides, options } of pending) {
      execute(paramOverrides, options);
    }
  }, [execute, failResolution, loadResult]);
