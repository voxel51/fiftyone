...
_OPERATOR_TO_PANEL_SURFACE = {
    PluginScope.DATASET_SAMPLES_GRID: "grid",
    PluginScope.DATASET_SAMPLE_MODAL: "modal",
    PluginScope.ALL: "portal",
}


def _normalize_panel_surfaces(
    surfaces: Union[PANEL_SURFACE, Iterable[Union[str, PluginScope]], None],
) -> str:
...
def _derive_scopes(
    ...
    A portal panel is explicitly available in :attr:`PluginScope.ALL`.
    """
    normalized = []
    for surface in panel_surfaces.split():
        scopes = _PANEL_TO_OPERATOR_SURFACES[surface]
        if PluginScope.ALL in scopes:
            return [PluginScope.ALL]
...
class PanelConfig(OperatorConfig):
    ...
    Args:
        ...
        surfaces ("grid"): the surfaces on which the panel can be displayed.
            ...
            :class:`PluginScope` values. A ``"portal"`` panel is not placed
            in a space; a consumer renders it explicitly
        scopes (None): the :class:`PluginScope` values in which the panel is
    ...
    def __init__(
        ...
        surfaces: Union[
            PANEL_SURFACE, Iterable[Union[str, PluginScope]]
        ] = "grid",
        scopes=None,
        priority=None,
        **kwargs
