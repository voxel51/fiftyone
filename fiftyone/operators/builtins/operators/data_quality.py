...
class ComputeExactDuplicates(foo.Operator):
    ...
    def execute(self, ctx):
        apply_fcn(ctx.dataset, _compute_filehash, "filehash")
...
def apply_fcn(samples, fcn, field, skip_failures=True, progress=None):
    with contextlib.ExitStack() as context:
        ...
        for sample in pb(samples):
            try:
                sample[field] = fcn(sample.local_path)
                ctx.save(sample)
...
def _compute_filehash(filepath):
    with Image.open(filepath) as image:
        return dhash(_convert_pillow_to_opencv(image))
