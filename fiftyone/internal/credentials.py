...
def _parse_credentials(creds_list, creds_dir, fernet, provider):
    ...
    file_name = ""
    if provider == "AWS":
        file_name = "aws_creds.ini"

    if provider == "GCP":
        file_name = "gcp_creds.json"

    if provider == "MINIO":
        file_name = "minio_creds.ini"

    raw_creds = fernet.decrypt(creds["credentials"]).decode()

    if "ini-file" in raw_creds:
        creds_path = os.path.join(creds_dir, file_name)
...
def _write_default_aws_ini_file(creds_dict, creds_path):
    ...
    config["default"] = {
        ...
        "region": creds_dict["default-region"],
    }
