...
def start(args_string):
    ...
    if context == _CONTEXT_NONE:
        ...
        handle = IPython.display.display(
            IPython.display.Pretty("Launching FiftyOne..."), display_id=True,
        )
    ...
    if isinstance(start_result, manager.StartLaunched):
        ...
        template = (
            "Reusing FiftyOne on port {port} (pid {pid}), started {delta} ago. "
        ...
        message = "ERROR: Failed to launch FiftyOne (exited with %d).%s%s" % (
