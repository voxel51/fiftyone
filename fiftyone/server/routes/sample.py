...
class SampleField(HTTPEndpoint):
    ...
    async def patch(self, request: Request, data: dict) -> dict:
        ...
        if is_delete:
            ...
            # Apply patches directly to the label
        ...
        if generated_dataset_name and generated_sample_id:
            try:
                ...
                generated_sample = generated_dataset[generated_sample_id]
                sync_to_generated_dataset(
                    generated_dataset, generated_sample, path, field_id, data
                )
                if not is_delete:
                    generated_sample.reload()
                    return utils.json.JSONResponse(
