...
class SampleTagsEndpoint(HTTPEndpoint):
    ...
    async def post(self, request: Request, data: dict) -> dict:
        ...
        sample_id = _get_required_path_param(request, "sample_id")
        tags = _tags_from_create_payload(data, sample_id=sample_id)
...
class TagCountsEndpoint(HTTPEndpoint):
    ...
    async def get(self, request: Request) -> dict:
        ...
        by_sample = request.query_params.get("by_sample", "").lower() == "true"
...
def _tags_from_create_payload(data, sample_id: str) -> list[fota.TemporalTag]:
...
def _tag_update_from_payload(data, *, sample_id: str, tag_id: str) -> dict:
    _require_dict(data, "request body")
    _reject_tag_update_fields(data)
...
def _reject_temporal_tag_timestamps(record: dict) -> None:
    ...
    # preserve provenance when they need to migrate existing records.
    ...
    if fields:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Tag {field_names} {verb} response-only",
        )


def _reject_tag_update_fields(record: dict) -> None:
    fields = {
        "anchor",
        "created_at",
        "created_by",
    ...
    if fields:
        ...
        raise HTTPException(
            status_code=400,
            detail=(
                f"Tag {field_names} {verb} not mutable through this route"
            ),
        )
