...
def get_zip_url(github_repo: str, ref: str = None):
    ...
    """
    if etaw.is_url(github_repo):
        repo_spec = parse_url(github_repo)

        # if "tree" in repo_spec:
        #     repo_spec.replace("tree", "zipball")
    else:
        repo_spec = parse_repo(github_repo)

    user = repo_spec.get("user")
    repo = repo_spec.get("repo")
    branch = repo_spec.get("branch")

    return f"https://api.github.com/repos/{user}/{repo}/zipball/{branch}"
...
class GitHubRepo:
    ...
    def list_repo_contents(self, recursive: bool = True) -> Optional[list]:
        ...
        content_url = f"https://api.github.com/repos/{self.user}/{self.name}/git/trees/{self.ref}?recursive=1"
