...
class DatasetView(foc.SampleCollection):
    ...
    def get_dynamic_group(self, group_value):
        ...
        if etau.is_str(group_expr):
            ...
            isinstance(group_expr, (list, tuple))
            and all(etau.is_str(e) and e.startswith("$") for e in group_expr)
            and isinstance(group_value, (list, tuple))
            and len(group_value) == len(group_expr)
        ):
            # match each field directly (index-eligible); an `$expr` over a computed array cannot use an index
            pipeline.append(
                {
                    "$match": {
                        field_ref[1:]: value
