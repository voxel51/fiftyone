"""
A seekable, read-only file object that reads one byte range at a time
through a fetcher, with bounded read-ahead.
...
#: Read-ahead for a streaming read. Callers and tests assert against what a
#: bounded read costs, so it is named rather than private to the reader.
DEFAULT_BLOCK_SIZE_BYTES = 4 * 1024 * 1024

#: The opening probe of the summary section at a file's tail.
DEFAULT_INDEX_BLOCK_SIZE_BYTES = 256 * 1024
...
class RangeReader(io.RawIOBase):
    ...
    @property
    def size(self) -> int:
        """The file's length in bytes, learned by the opening probe."""
        return self._size
