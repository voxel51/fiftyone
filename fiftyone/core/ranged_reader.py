...
The existing storage helpers are shaped for local files: they hand back whole
objects, which costs nothing on a disk and transfers an entire shard over a
network. This reads a range at a time instead, so a footer or one row group
costs its own bytes rather than the object's.

Buffering, seeking and read-ahead live in :class:`RangeReader`; how bytes are
obtained is a fetcher's business. An installation that can reach files this
process cannot open supplies its own fetcher, so every caller sees one
contract whatever is behind it.
