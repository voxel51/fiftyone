...
def load_image_classification_dataset(
    ...
    Args:
        ...
        backing_dir (None): an optional backing directory in which store
            ...
            generated by :func:`get_default_backing_dir`
...
def load_image_detection_dataset(
    ...
    Args:
        dataset: the iterable dataset
        labels_map (None): an optional dict mapping class IDs to class strings.
            If provided, it is assumed that the ``label``s in ``target`` are
            class IDs that should be mapped to class strings via
            ``labels_map[target]``
        sample_parser (None): an optional
            :class:`fiftyone.core.datautils.ImageDetectionSampleParser` to use
...
def load_labeled_image_dataset(
    ...
    Args:
        dataset: the iterable dataset
        sample_parser (None): an optional
            :class:`fiftyone.core.datautils.LabeledImageSampleParser` to use
            to parse the provided samples
        name (None): a name for the dataset. By default,
            :func:`get_default_dataset_name` is used
        backing_dir (None): an optional backing directory in which store
