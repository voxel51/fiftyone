...
def apply_model(samples, model, label_field, confidence_thresh=None):
    ...
    return _apply_image_model(samples, model, label_field, confidence_thresh)
...
def compute_embeddings(samples, model, embeddings_field=None):
    ...
    return _compute_image_embeddings(samples, model, embeddings_field)


def _compute_image_embeddings(samples, model, embeddings_field):
    # Use data loaders for Torch models, if possible
    if isinstance(model, TorchModelMixin):
        # Local import to avoid unnecessary Torch dependency
        import fiftyone.utils.torch as fout
