...
class AppleSharpModel(fout.TorchImageModel):
    ...
    def _export_gaussians(self, gaussians, f_px, height, width):
        ...
        splat_path = os.path.join(self._output_dir, f"splat_{splat_id}.ply")
        ...
        scene.add(
            fo3d.GaussianSplat(
                "gaussians",
                os.path.basename(splat_path),  # relative to .fo3d location
                format="ply",
                center_geometry=True,
            )
        )
        scene_path = os.path.join(self._output_dir, f"scene_{splat_id}.fo3d")
    ...
    def _predict_all(self, imgs):
        ...
        with torch.no_grad():
            for img in imgs:
                ...
                fl_mm = (
                    exif_fl if exif_fl is not None else self._focal_length_mm
                )
                ...
                img_pt = (
                    torch.from_numpy(img_np.copy()).float().to(self._device)
                )
