...
def resize_pos_embed(
    ...
    logging.info(
        "Resizing position embedding grid-size from %s to %s",
        old_grid_size,
        grid_size,
    )
...
def resize_text_pos_embed(
    ...
    logging.info(
        "Resizing text position embedding num_pos from %s to %s",
        old_num_pos,
        num_pos,
    )
    old_pos_embed = old_pos_embed.reshape(1, old_num_pos, old_width).permute(
        0, 2, 1
    )
...
def set_model_preprocess_cfg(model, preprocess_cfg: Dict[str, Any]):
    ...
    module.image_std = preprocess_cfg[
        "std"
    ]  # legacy attribute, keeping for bwd compat
