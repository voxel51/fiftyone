...
def _rescan_model_configs():
    ...
    for cf in config_files:
        with open(cf, "r") as f:
            ...
            if all(
                a in model_cfg for a in ("embed_dim", "vision_cfg", "text_cfg")
            ):
...
def get_tokenizer(
    ...
    **kwargs,
    ...
    if model_name.startswith(HF_HUB_PREFIX):
        ...
        assert (
            config is not None
        ), f"No valid model config found for {model_name}."
...
def create_model(
    ...
    **model_kwargs,
    ...
    if has_hf_hub_prefix:
        ...
        pretrained_hf = (
            False  # override, no need to load original HF text weights
        )
    else:
