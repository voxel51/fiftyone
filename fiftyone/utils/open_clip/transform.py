...
def merge_preprocess_dict(
    ...
    if overlay:
        overlay_clean = {
            ...
            if k in _PREPROCESS_KEYS and v is not None
        }
...
class ResizeKeepRatio:
    ...
    def __init__(
        ...
        self.longest = float(
            longest
        )  # [0, 1] where 0 == shortest edge, 1 == longest
    ...
    def get_params(
        ...
        if random_scale_prob > 0 and random.random() < random_scale_prob:
            ratio_factor = random.uniform(
                random_scale_range[0], random_scale_range[1]
            )
        ...
        size = [
            round(x * f / ratio) for x, f in zip(source_size, ratio_factor)
        ]
        return size
...
class color_jitter(object):
    ...
    def __init__(
        self, brightness=0.0, contrast=0.0, saturation=0.0, hue=0.0, p=0.8
    ):
        assert 0.0 <= p <= 1.0
        self.p = p
    ...
    if is_train:
        ...
        if use_timm:
            ...
            if aug_cfg.color_jitter_prob:
                assert (
                    aug_cfg.color_jitter is not None
                    and len(aug_cfg.color_jitter) == 4
                )
                train_transform.extend(
                    [
                        color_jitter(
                            *aug_cfg.color_jitter, p=aug_cfg.color_jitter_prob
                        )
                    ]
                )
