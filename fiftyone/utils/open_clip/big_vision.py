...
def load_big_vision_weights(model: CustomTextCLIP, checkpoint_path: str):
    ...
    def _convert_timm_img(module, prefix):
        ...
        for i, block in enumerate(module.blocks.children()):
            ...
            mha_prefix = (
                block_prefix + f"MultiHeadDotProductAttention_{mha_sub}/"
            )
        ...
        module.norm.weight.copy_(
            _n2p(w[f"{prefix}Transformer/encoder_norm/scale"])
        )
        module.norm.bias.copy_(
        ...
        if module.attn_pool is not None:
            ...
            module.attn_pool.latent.copy_(
                _n2p(w[f"{block_prefix}probe"], t=False)
            )
            ...
            module.attn_pool.norm.bias.copy_(
    ...
    def _convert_openclip_transformer(module: Transformer, prefix):
        for i, block in enumerate(module.resblocks.children()):
            ...
            block.ln_2.weight.copy_(
                _n2p(w[f"{block_prefix}LayerNorm_1/scale"])
            )
            ...
            block.mlp.c_fc.bias.copy_(
                _n2p(w[f"{block_prefix}MlpBlock_0/Dense_0/bias"])
            )
    ...
    def _convert_openclip_txt(module: TextTransformer, prefix):
        ...
        _convert_openclip_transformer(
            module.transformer, prefix=prefix + "Encoder_0/"
        )
        module.ln_final.weight.copy_(
            _n2p(w[f"{prefix}Encoder_0/encoder_norm/scale"])
        )
        module.ln_final.bias.copy_(
            _n2p(w[f"{prefix}Encoder_0/encoder_norm/bias"])
        )
