...
class CoCa(nn.Module):
    def __init__(
        ...
        text_cfg = (
            CLIPTextCfg(**text_cfg) if isinstance(text_cfg, dict) else text_cfg
        )
        vision_cfg = (
            CLIPVisionCfg(**vision_cfg)
        ...
        vocab_size = (
            ...
            if hasattr(text_cfg, "hf_model_name")
            and text_cfg.hf_model_name is not None
    ...
    def _encode_image(self, images, normalize: bool = True):
        ...
        image_latent = (
            F.normalize(image_latent, dim=-1) if normalize else image_latent
        )
    ...
    def _encode_text(self, text, normalize: bool = True):
        ...
        text_latent = (
            F.normalize(text_latent, dim=-1) if normalize else text_latent
        )
    ...
    def generate(
        ...
        with torch.no_grad():
            ...
            pad_token_id = (
                self.pad_id if pad_token_id is None else pad_token_id
            )
            ...
            while True:
                ...
                if mask.all():
                    ...
                    filtered_logits = logit_warper(
                        x[~mask, :], filtered_logits
                    )
    ...
    def _generate_beamsearch(
        ...
        logits_processor = (
            LogitsProcessorList(
                [
                    MinLengthLogitsProcessor(
                        min_seq_len, eos_token_id=eos_token_id
                    )
                ]
            )
