...
def freeze_batch_norm_2d(module, module_match={}, name=""):
    ...
    if is_match and isinstance(
        module,
        (nn.modules.batchnorm.BatchNorm2d, nn.modules.batchnorm.SyncBatchNorm),
    ):
        ...
        for child_name, child in module.named_children():
            full_child_name = (
                ".".join([name, child_name]) if name else child_name
            )
...
def replace_linear(
    ...
    for name, module in model.named_children():
        if len(list(module.children())) > 0:
            replace_linear(
                module, linear_replacement, include_modules, copy_weights
            )
