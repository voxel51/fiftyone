...
def annotate(
    ...
    **kwargs,
    ...
    api = LabelboxAnnotationAPI(auth=auth, url=url, https=https, port=port)
...
def load_annotations(info):
    ...
    annotations = api.download_annotations(label_field, project_id)
    return annotations


class LabelboxAnnotationAPI(foua.BaseAnnotationAPI):
    """Basic interface for connecting to Labelbox, sending samples for
    annotation, and importing them back into the collection.
    """

    def __init__(
        self, auth=None, url="labelbox.com", https=True, port=None,
    ):
        self._auth = auth
        ...
        self._client = lb.client.Client(
            api_key=self._auth, endpoint=self.base_graphql_url
        )

    def get_api_key(self):
        if self._auth is None:
            self._auth = fo.annotation_config.labelbox_api_key
        if self._auth is None:
            logger.info(
                "To not enter the API key every time `annotate()` is "
                "called, provide the `auth` keyword "
                "argument to `annotate()` or specify the "
                "`FIFTYONE_LABELBOX_API_KEY` environment variable"
            )
            self._auth = self.prompt_api_key("Labelbox")

    @property
    def base_api_url(self):
        return "%s://api.%s%s" % (self._protocol, self._url, self._port)
    ...
    def upload_data(self, samples):
        upload_info = []
        for sample in samples:
            item_url = self._client.upload_file(sample.filepath)
            upload_info.append(
                {
        ...
        task = self._dataset.create_data_rows(upload_info)
        task.wait_till_done()
        return task

    def create_ontology_tools(self, classes, label_type, attributes):
        from labelbox.schema.ontology import Classification, Tool

        tools = []
        # TODO: Use polygons for instance segmentation
        if label_type in (fol.Detection, fol.Detections):
            tool_type = Tool.Type.BBOX
        elif label_type in (fol.Keypoint, fol.Keypoints):
            tool_type = Tool.Type.POINT
    ...
    def upload_samples(self, samples, label_field=None, classes=None):
        ...
        # With https://github.com/voxel51/fiftyone/pull/1137
        # attributes = list(dataset.schema(label_field, dynamic_only=True).keys())

        attributes = set()
        sample_labels = samples.values(label_field)
        for sample_label in sample_labels:
            ...
            for label in labels:
                ...
                attributes.union(set(new_fields) - set(default_fields))

        attributes = list(attributes)

        tools = self.create_ontology_tools(
        ...
        self._dataset = self._client.create_dataset(
            name="FiftyOne-annotations"
        )
        task = self.upload_data(samples)
        ...
        project = self._client.create_project(name="FiftyOne-annotations")
        project.setup(editor, ontology_builder.asdict())
        project.datasets.connect(self._dataset)
...
class LabelboxAnnotationInfo(foua.AnnotationInfo):
    def __init__(self, label_field, api, project_id):
        super().__init__(label_field=label_field, backend="labelbox")
        self.api = api
