...
class COCOeval:
    ...
    def accumulate(self, dataset, sample_id_map, p = None):
        '''
        ...
        sample_p    = -np.ones((len(p.imgIds),T,R,K,A,M))
        sample_r    = -np.ones((len(p.imgIds),T,K,A,M))
        sample_s    = -np.ones((len(p.imgIds),T,R,K,A,M))
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    for e in E:
                        ...
                        for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                            ...
                            if nd:
                                sample_r[e['image_id'],t,k,a,m] = rc[-1]
                            else:
                                sample_r[e['image_id'],t,k,a,m] = 0
                            ...
                            sample_p[e['image_id'],t,:,k,a,m] = np.array(q)
                            sample_s[e['image_id'],t,:,k,a,m] = np.array(ss)
                            
                                    


                    # Compute precision and recall for all detections of a
                    # category in the entire dataset
        ...
        toc = time.time()
        print('DONE (t={:0.2f}s).'.format( toc-tic))
        return sample_p, sample_s, sample_r
