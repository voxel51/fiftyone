...
Vendored from Pointcept (https://github.com/Pointcept/Pointcept), released under
the MIT License. See ``fiftyone/utils/ptv3_arch/__init__.py`` for attribution.

Author: Xiaoyang Wu (xiaoyang.wu.cs@gmail.com)
Please cite Point Transformer V3 (CVPR 2024) if this code is helpful to you.
"""
...
# PDNorm is only used by Point-Prompt-Training (PPT) variants. The pretrained
# backbones wrapped by the FiftyOne zoo do not use it, so it is left unset here.
PDNorm = None
...
class SerializedAttention(PointModule):
    ...
    def forward(self, point):
        if not self.enable_flash:
            self.patch_size = min(
                offset2bincount(point.offset).min().tolist(),
                self.patch_size_max,
            )
