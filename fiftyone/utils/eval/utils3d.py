...
def _polygon_clip(subject_polygon, clip_polygon):
    ...
    def _inside(p):
        s1 = (cp2[0] - cp1[0]) * (p[1] - cp1[1])
        s2 = (cp2[1] - cp1[1]) * (p[0] - cp1[0])
        return s1 > s2
    ...
    for clip_vertex in clip_polygon:
        ...
        for subject_vertex in input_list:
            ...
            if _inside(e):
                if not _inside(s):
                    output_list.append(_compute_intersection())
...
def _is_clockwise(p):
    """
    Checks if orientation is clockwise
    """
    x = p[:, 0]
    y = p[:, 1]
...
def _compute_3d_bbox_iou(gt, gt_crowd, pred):
    ...
    inter, inter_area = _convex_hull_intersection(rect1, rect2)
    ...
    vol1 = _box3d_vol(corners1)
    vol2 = _box3d_vol(corners2)
