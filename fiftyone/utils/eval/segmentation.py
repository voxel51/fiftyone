    ...
    def evaluate_samples(
        ...
        Args:
            ...
            num_workers (1): the number of processes to use for parallel
                ...
                processing
            batch_method ("id"): the method to use to shard the dataset
            batch_size (None): the number of samples to process in each batch.
                If not provided, the batch size is set to the number of samples
                in the dataset divided by the number of workers.
            parallelize_method (None): the parallelize_method to use for multiprocessing
...
class SimpleEvaluation(SegmentationEvaluation):
    ...
    def evaluate_samples(
        ...
        acc_field = "%s_accuracy" % eval_key if save else None
        pre_field = "%s_precision" % eval_key if save else None
        rec_field = "%s_recall" % eval_key if save else None
