...
class FiftyOneDataset(Dataset):
    ...
    where ``info.json`` is an optional JSON file containing info associated
    with the dataset, and ``samples.json`` is a JSON file containing a
    serialized representation of the samples in the dataset generated by
